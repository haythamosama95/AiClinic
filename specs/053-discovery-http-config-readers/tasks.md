...
- [X] T006 [US1] Add named test `T5 discovery_http_unauthenticated` to `ai-platform/test/discovery-http.test.ts`: missing/invalid/non-Bearer AAT → taxonomy `unauthenticated`; no manifest body; no `ai_request` journal row (FR-002, FR-005 / SC-003; §5.5; §4.3.2; §6.4). Fails red until the handler verifies Bearer AAT with the same verifier as submit. **Satisfies**: FR-002, FR-005 / SC-003. **Proves**: T5.
...
- [X] T010 [US1] Add named test `T9 config_reader_presence_grants_lifecycle_overlay` to `ai-platform/test/config-readers.test.ts`: present installation grant **and** global-scope lifecycle overlay (`global/{capabilityId}/{version}`) served through the production D1 config reader (FR-007 / SC-004; §4.3.2; Done when). Fails red until grants presence is assertable. **Satisfies**: FR-007 / SC-004. **Proves**: T9.
- [X] T011 [US1] Add named test `T10 config_reader_presence_kill_switches` to `ai-platform/test/config-readers.test.ts`: present kill_switch rows for cache keys `global` and `{scope}:{target}` (capability / installation / provider) served through the production D1 config reader (FR-007 / SC-004; §4.3.2; §7.3). Fails red until the additive `kill_switch` table and SELECT exist. **Satisfies**: FR-007 / SC-004. **Proves**: T10.
...
- [X] T020 [US1] From `ai-platform/`, run this slice's Workers suites — `npx vitest run --config vitest.workers.config.ts test/discovery-http.test.ts test/config-readers.test.ts`. Then run every prior slice's suite: `npx vitest run` (default Node-pool prior suites) and `npx vitest run --config vitest.workers.config.ts` (workers-pool prior suites). Confirm I2's fourteen named cases (T1–T14) are green and every prior suite stays green. No new test is added here — this is the §3.10 checkpoint gate. Confirm inherited prohibitions hold: no per-request server-side state; no second Quota DO / R2; no journal row for auth failure on this surface; no §9.14 mechanism (§6.4). **Satisfies**: the §3.10 checkpoint rule (T1–T14 + prior suites). Proved by itself.
