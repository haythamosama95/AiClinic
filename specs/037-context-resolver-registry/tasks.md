...
**Prerequisites**: `plan.md` (required), `spec.md` (required for the user story). `research.md` is never produced on this platform (the research is `docs/architecture/ai-platform/01-ai-platform.md`). `data-model.md` is not produced — E3 defines no entities (spec Key Entities: not applicable). `contracts/` Freezes artifacts are named in the plan Files section and written/confirmed in Phase 4 (Documentation). `quickstart.md` is written in Phase 4.
