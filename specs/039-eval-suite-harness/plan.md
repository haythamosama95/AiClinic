...
**Primary Dependencies**: The existing `ai-platform/` Worker tree (Vitest ~3.2, Wrangler). Harness lives under `ai-platform/test/eval/` only — no `src/eval/` module (Clarification Q1). Golden cases bind to D1 prompt artifacts/composer and D5 recorded DeepSeek fixtures under `test/fixtures/deepseek/` via `d5_fixture_subdir` behind the D2 provider port. Live smoke invokes wired adapters (`runLiveSmokeSuite`) against pinned `model_id` values from the platform routing-policy document (never floating aliases), writes `run_kind: "live_smoke"` reports, and is scheduled by a GitHub Actions workflow that supplies provider secrets (Clarification Q3). Credential-less local runs may skip live egress. No new external package is required beyond the existing Vitest stack.
...
**Testing**: Vitest (`npx vitest run`) at the §13.5 **Capability evals (A9)** layer / delivery plan §3.11.6 row F1 ("CI"). Named tests T1–T8 live under `ai-platform/test/eval/`. Goldens use recorded D5 fixtures (no permanent live egress) and gate composition + schema (fixture-output sanity only on canned bodies). Live smoke is the smaller scheduled set that performs live adapter egress against pinned models (local skip only when credentials absent). Empty case sets overall-fail. Pass/fail only — no numeric score cutoff (Clarification Q5; Session 2026-08-05; A9 / §13.5).
...
└── test/
    └── eval/                                         # NEW — harness root only (Clarification Q1; no src/eval/)
        ...
        │   ├── fixtures/                             # NEW — capability-scoped bindings (`d5_fixture_subdir` → `test/fixtures/deepseek/`); no eval-local provider-response duplicates (Consumes D5; FR-002, FR-009)
        │   └── expectations/                         # NEW — composition needles / request-golden + fixture-output sanity + schema (Clarification Q5; Session 2026-08-05; FR-004)
        ├── prompts/
...
| From D5 — first real provider adapter and its recorded-fixture adapter suite behind the D2 provider port (wire mapping, stream normalization, usage extraction, error classification proven against recorded fixtures) | `ai-platform/src/provider/deepseek.ts` (`DeepSeekAdapter`); `ai-platform/src/provider/port.ts` (`ProviderPort`); recorded fixtures under `ai-platform/test/fixtures/deepseek/`; frozen in `specs/032-first-real-provider-adapter/contracts/first-real-provider-adapter.md`. F1's golden cases resolve `d5_fixture_subdir` against `ai-platform/test/fixtures/deepseek/` (provider bodies live in the D5 tree, not under `test/eval/`); F1 does not redefine the adapter port, invent a second real adapter, or replace D5's fixture-suite duties. Live smoke invokes adapters against live endpoints for pinned product/version `model_id` values (workflow-supplied secrets; `run_kind: "live_smoke"`) without changing adapter ownership |
...
| `ai-platform/test/eval/clinic.visit_summary/expectations/**` | FR-004 (composition needles / request-golden + fixture-output sanity + schema validation; Clarification Q5 / Session 2026-08-05) |
...
Every named test in the spec's Test plan is placed in a §13.5 layer (stop condition 3 not triggered). F1 emits **no** §5.4 taxonomy codes — pass/fail of goldens and completion of scheduled smoke are build/schedule outcomes (spec Test plan coverage note). T2 expects failure when the golden set is run against the checked-in worse prompt artifact (Clarification Q4), not against the production prompt. T4 asserts the smoke entry targets pinned product/version `model_id` values from routing policy (never floating aliases), that scheduled/live execution invokes adapters (or skips only when credentials are absent locally), and that a `run_kind: "live_smoke"` report is written when smoke runs (Clarification Q3; Session 2026-08-05). T5 spies/asserts that golden execution resolves D5 `test/fixtures/deepseek/` via `d5_fixture_subdir` rather than live egress or eval-local duplicates. T7 asserts this slice introduces no Flutter client files carrying prompt/provider/model strings. T8 asserts the harness introduces no per-request server-side state.
