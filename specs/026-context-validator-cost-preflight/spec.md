...
- **Error codes this slice can emit** (closed set, §5.4; stages 6–7 emit no others, §6.1): `context_required` (422, retryable after resolving, no quota), `context_invalid` (422, not retryable, no quota), `request_too_large` (413, not retryable, no quota).
...
- **Undeclared key that happens to match a declared key's name with a different version**: a key is identified by its full `domain.concept@vN` name; a same-concept different-version key is undeclared and is dropped (§5.2 Key format, Evolution).
- **Branch consistency vs. shape**: an org/branch mismatch is a tenant-consistency failure, not a shape failure, but it shares the stage's non-`context_required` code `context_invalid` by elimination from §6.1 stage 6's failure column; it is NOT reported as `forbidden_capability` (that is the entitlement stage's code, stage 3) and NOT as `request_too_large`.
...
- **FR-006**: The context-validate stage (stage 6) MUST emit only `context_required` or `context_invalid`, and the cost pre-flight stage (stage 7) MUST emit only `request_too_large`; both stages are CPU-only and perform no I/O. (§6.1 stages 6–7)
- **FR-007**: The cost pre-flight MUST compute `estimatedInputTokens = ceil(utf8ByteLength(serialized request input) / 4) * 1.15` over the filtered context plus the caller's intent, and MUST reject with `request_too_large` when `estimatedInputTokens + maxOutputTokens > perRequestCostCeiling` or when `estimatedInputTokens > maxInputTokens`, before any egress to a provider. The comparison is in tokens; C2 MUST NOT apply any token→cost conversion and MUST NOT hold any provider price. (§4.3.3, §6.1 stage 7, §5.1 Economics, §13.6.2)
...
- **H2 (conversational transcript validation)**: §4.3.5's conversational paragraph — the permitted-key-set enforcement and transcript budget — is band H, not C2. C2 implements the `single_shot` context validation only. An `interaction_mode: conversational` manifest is not validated by this slice. (§4.3.5 conversational paragraph, §6.7)
