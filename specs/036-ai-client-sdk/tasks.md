...
- [X] T006 [US1] Add named test `sdk_cancel_closes_stream` (T6) to `frontend/test/unit/core/ai/ai_client_sdk_test.dart`: cancel closes the in-flight stream; spy asserts no separate cancel endpoint is called (§5.5 Cancel row and rule 5; §4.1; §3.11.5 E2). **Satisfies**: FR-008 / SC-003. **Proves**: T6.
...
- [X] T010 [US1] Add named test `sdk_transport_retry_allowed` (T26) to `frontend/test/unit/core/ai/ai_client_sdk_test.dart`: a transport failure before a terminal platform event is retried by the SDK with the same idempotency key (T4 companion; §4.1). **Satisfies**: FR-001, FR-005 / SC-002. **Proves**: T26.
