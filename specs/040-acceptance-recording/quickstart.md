...
registry, `public.ai_accepted_output`, and the Flutter clinical accept/discard path. Acceptance is
entirely clinic-side — no gateway or D1 journal involvement.
...
- **Spec delivered:** Single shared `record_ai_acceptance` RPC, registry + demonstration target `visit_clinical_notes` → `save_visit_documentation`, atomic domain + `ai_accepted_output` + `ai.acceptance_record` audit, Flutter accept/discard without promoting `advisory_display`.
- **Plan scoped:** One migration, Flutter `acceptance/` module family, SQL + Flutter test suites T1–T10, frozen contract at `contracts/acceptance-recording.md`.
...
- `ai_internal.acceptance_targets` — registry with demonstration row `visit_clinical_notes`.
- `public.ai_accepted_output` — stores the AI request reference handle only (no prompts/providers/models).
- Flutter clinical accept path — `ClinicalAcceptController` invokes the RPC on explicit accept; discard writes nothing.
- E4 `advisory_display` accept remains non-writing (T8).
...
| `frontend/lib/features/ai/acceptance/clinical_accept_controller.dart` | Accept/discard orchestration |
...
- Local Supabase PostgreSQL with migrations applied (including `20260802150000_ai_acceptance_recording.sql`).
...
Optional: with a local visit in progress, drive the clinical accept harness against
