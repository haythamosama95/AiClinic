...
**Prerequisites**: `plan.md` (required), `spec.md` (required for the user story). `research.md` is never produced on this platform (the research is `docs/architecture/ai-platform/01-ai-platform.md`). `data-model.md` is not produced — F2 defines clinic-side Supabase entities, not D1 entities (entity shapes live in `contracts/acceptance-recording.md`). `contracts/acceptance-recording.md` is already frozen on disk (written during the plan phase per DP-4). `quickstart.md` is written in Phase 4 (Documentation).
