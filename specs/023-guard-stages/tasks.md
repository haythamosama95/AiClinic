...
- [X] T015 [US1] Run `npx vitest run --config vitest.workers.config.ts` (this slice's three files + B2's `control.test.ts`) and then `npx vitest run` (the default Node-pool config covering every prior band-A/band-B1/band-B2 suite — `config-cache.test.ts` (A5), `context.test.ts` (A5), `migrations.test.ts` (A5), `manifest.test.ts` (A4), `canonical.test.ts` (A3), `taxonomy.test.ts`, `reference.test.ts`, `error-body.test.ts`, `trace.test.ts`, `log-redaction.test.ts` (A2), `health.test.ts`, `env-deploys.test.ts` (A1), `adapter.test.ts` (A6)). Confirm every prior suite stays green. No new test is added here — this is the §3.10 checkpoint gate, not extra work. **Satisfies**: the §3.10 checkpoint rule.
