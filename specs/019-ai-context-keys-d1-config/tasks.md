...
- [X] T004 [US1] Add `T-A5-03 context_key_storage_named_rejected` to `ai-platform/test/context.test.ts`: `validateKey("visits_vitals_table@v1")`, `validateKey("get_visit_vitals_rpc@v1")`, and well-formed storage-shaped keys such as `visits.vitals_view@v1` are rejected with `storage_named_key` — a key named after storage rather than meaning is rejected even when the referenced artefact would exist. Satisfies FR-002 / SC-001.
- [X] T005 [US1] Add `T-A5-04 context_key_unknown_version_rejected` and `T-A5-04b context_key_unknown_key_rejected` to `ai-platform/test/context.test.ts`: `validateKey("visit.vitals@v9")` for an unpublished version of a known concept is rejected as `unknown_version`; a well-formed key outside the vocabulary is rejected as `unknown_key`. Satisfies FR-006 / SC-001.
...
- [X] T017 [US1] Add `T-A5-15 request_reference_index_exists_and_unique` to `ai-platform/test/migrations.test.ts`: the `ai_request` request-reference index exists and is unique, and the column stores the A2 format unchanged (asserted via the index DDL plus insert/select of a `generateRequestReference()` value). Also `T-A5-15b` asserts there is no unique index on `(installation_id, idempotency_key)` — C3 Quota DO owns idempotency (§4.3.3). Satisfies FR-011 / SC-004.
