...
- The **get-request endpoint response contract**: an authenticated (`Authorization: Bearer <AAT>`, §5.6 / B3 `EnrolledKeyVerifier`), installation-scoped read resolves a request reference through one indexed D1 lookup on `request_reference` to the journaled state — for a completed request with a readable envelope, the validated result from the R2 envelope's `result` section; for completed-without-envelope, state only; for failed, state plus terminal error code and no content; for cancelled or `AwaitingContext`, state only; for in-flight states, `{ state, pending: true }`; unknown or installation mismatch → not found; missing/invalid token → 401 `unauthenticated` (§5.5, §7.6, §7.4).
...
As the AI Gateway Worker, once the guard has passed and the request has been admitted (B4), I create the durable `ai_request` row synchronously at stage 9 before any inference work begins, journal every §6.3 state transition with a timestamp (terminal states immutable once reached), and update the row with its terminal state at stage 15; after the terminal event has been emitted to the client, I write the per-attempt `ai_attempt` rows, exactly one `usage_event` ledger row, and exactly one R2 payload envelope (keyed `request/{id}/envelope`, containing the four sections `context`, `prompt`, `attempts[]`, `result`) in the post-response continuation at stage 16, and a failure in that continuation never fails the already-completed request. A request rejected by the guard produces no journal row (rejection counted by rate-limit/admission). Through the authenticated get-request endpoint, a request reference resolves to its journaled state — and, if completed with a readable envelope, the validated result — via a single indexed D1 lookup scoped to the caller's installation.
...
3. **Given** a request rejected by the guard, **When** the guard rejects it, **Then** no `ai_request` row is created (spy on D1 inserts: zero for the request) — the rejection is counted in `platform_counter` by the rate-limit/admission path, not journaled as a request. *(rejected — no row)*
4. **Given** any §6.3 state transition, **When** the transition occurs, **Then** it is journaled with a timestamp, so the state timeline is reconstructable from the row; a second terminal write is a no-op (immutability). *(every §6.3 transition timestamped; terminal immutable)*
...
11. **Given** a completed request with a readable envelope, **When** the authenticated get-request endpoint is called with its reference, **Then** it returns the terminal state plus the validated result, via exactly one indexed D1 lookup (plus the one R2 `GetObject` on `payload_pointer`). *(get-request completed)*
...
14. **Given** an unknown reference (or installation mismatch), **When** the get-request endpoint is called with it, **Then** it returns not found, via the indexed lookup. *(get-request unknown)*
15. **Given** a request in `AwaitingContext`, **When** get-request is called with a valid AAT for that installation, **Then** it returns 200 with `state` only and no R2 read. *(get-request AwaitingContext)*
16. **Given** a get-request without a valid Bearer AAT, **When** the endpoint is called, **Then** it returns 401 `unauthenticated`. *(get-request auth)*
17. **Given** a completed request whose `payload_pointer` is null or whose envelope is missing/corrupt, **When** get-request is called, **Then** it returns 200 `{ state: "Completed" }` without `result` (not 404). *(get-request completed resultMissing)*
...
| T5 | `guard_rejected_produces_no_row` | a guard-rejected request produces zero `ai_request` inserts (spy); the rejection is counted in `platform_counter` via the rate-limit/admission live path — not journal-module tally helpers (§6.2, §7.5, §7.3) |
| T6 (one per transition) | `every_state_transition_timestamped` | each §6.3 transition (Accepted, Composing, Invoking, Streaming, Validating, Repairing, AwaitingContext, Completed, Failed, Cancelled) is journaled with a timestamp; terminal writes are immutable (second write no-op); the state timeline is reconstructable (§4.3.11, §6.3, §8.9) |
...
| T14 | `get_request_completed_returns_state_and_result` | a completed request's reference returns state plus the validated result, via one indexed D1 lookup (§5.5, §7.6, §7.4); requires valid AAT |
...
| T17 | `get_request_unknown_reference_returns_not_found` | an unknown reference (or installation mismatch) returns not found, via the indexed lookup (§5.5, §7.6) |
...
Coverage of every error code the slice can emit (§3.10 item 2): C3's own stages (9, 15, 16) emit only `internal_error` on a stage-9 D1 insert failure (§6.1 stage 9), returned with `request_reference` and `trace_id`; stage 16 emits no error to the client by construction ("never fails the request"). Terminal error codes written to the row (`completed`/`failed`/`cancelled` and the taxonomy codes carried by `failed`) are produced by later stages (D3/D4/D6) and are recorded, not generated, by C3 — `Failed` requires a taxonomy code. The get-request endpoint returns not found for an unknown reference or installation mismatch (response state, not a taxonomy error code) and 401 `unauthenticated` for a missing/invalid AAT.
...
- **Guard rejection produces no row**: a request rejected at any guard stage (1–8) is counted in `platform_counter` by rate-limit/admission and is never journaled as a request — putting a D1 write in the cheap-rejection path is the named anti-pattern (§6.2, §7.5). The journal module does not own the tally helpers. The `rejected` terminal state is therefore asserted by the *absence* of a row, not by a row in that state.
- **Stage 9 precedes prompt composition (stage 10)**: the `ai_request` row's prompt artifact hash is the one pinned by the resolved capability manifest (§5.7, A4), not one computed from a composed prompt, so it is known at stage 9 before stage 10 runs. C3 does not compose the prompt. Stage-9 insert may also record optional `routing_tier` (`standard` \| `degraded`).
...
- **Post-response failure isolation**: an R2 `PutObject` failure or a D1 insert failure at stage 16 must not surface to the client or change the terminal state already emitted; failures are logged via `console.error` with `request_id`; attempt+usage_event use `db.batch`; R2-without-pointer is an accepted partial state (§6.1, §4.3.11).
- **Get-request result source**: the validated result lives in the R2 envelope's `result` section (§7.4); D1 holds payload pointers, not result text (§7.3, §7.5). Get-request performs one indexed D1 lookup (state + pointer, scoped to installation) and, only when `payload_pointer` is non-null, one R2 `GetObject`; missing/corrupt envelope → `Completed` without `result`. Failed, cancelled, `AwaitingContext`, and in-flight requests perform no R2 read.
- **Get-request authentication**: requires `Authorization: Bearer <AAT>` verified via §5.6 / B3 `EnrolledKeyVerifier`; missing/invalid → 401 `unauthenticated`; installation mismatch → same as unknown → 404.
...
- **`AwaitingContext` and `Repairing` transitions**: these §6.3 states are reached only by later slices (H1 conversational, D6 repair), but the journal writer is generic over §6.3 and must timestamp them when they occur (including `completed_at` for `AwaitingContext`); terminal states are immutable. C3's tests drive them synthetically to prove the mechanism without implementing the behaviours that reach them.
- **Conversational columns**: for a `single_shot` request the nullable `conversation_id` / `turn_ordinal` columns (A5) are written null; C3 writes them from the request when present and does not introduce a conversation entity (§7.3, §6.7 — the conversational *behaviour* is band H). H3 helpers (`listConversationLegs`, `canReachAwaitingContext`, transition guards) are part of the journal export surface.
...
- **FR-002**: The `ai_request` row created at stage 9 MUST carry the request id, request reference, installation, actor, branch, capability id+version, the prompt artifact hash pinned by the resolved manifest, idempotency key, trace id, the initial `Accepted` state, optional `routing_tier` (`standard` \| `degraded`), and — when present — `conversation_id` / `turn_ordinal`; payload pointers are null until stage 16. (§4.3.11, §7.3, §5.7)
- **FR-003**: Every state transition in §6.3 MUST be journaled with a timestamp, so the state timeline is reconstructable from the row; `Completed`, `Failed`, `Cancelled`, `Rejected`, and `AwaitingContext` are terminal and immutable — writers MUST no-op when the row is already terminal. (§4.3.11, §6.3, §8.9)
- **FR-004**: At stage 15 the journal writer MUST update the `ai_request` row with the terminal state and the terminal error code (when failed); a `Failed` write MUST require a taxonomy code; this update is to an already-durable row and MUST NOT emit an error to the client. (§6.1 stage 15, §4.3.11)
- **FR-005**: A request rejected by any guard stage (1–8) MUST produce no `ai_request` row; the rejection MUST be counted in `platform_counter` by rate-limit/admission instead, never journaled as a request. (§6.2, §7.5, §7.3)
...
- **FR-008**: In stage 16 the journal writer MUST write exactly one `ai_attempt` row per provider attempt, and exactly one `usage_event` row per request — never one row per stream chunk and never more than one `usage_event`. Attempt + usage_event writes MUST use `db.batch`. (§7.3, §6.1 stage 16, §7.5)
- **FR-009**: Stage 16 MUST run entirely after the terminal event has been emitted to the client, and a failure in stage 16 (R2 `PutObject` or D1 insert) MUST NOT fail the request or change the terminal state already emitted; failures MUST be logged via `console.error` with `request_id`. (§6.1 stages 14–16, §4.3.11, §3.11.3)
- **FR-010**: The get-request endpoint MUST require a valid AAT (`Authorization: Bearer`, §5.6 / B3 `EnrolledKeyVerifier`), scope lookup to `principal.installationId`, and resolve a request reference through a single indexed D1 lookup on `request_reference`; for a completed request with a readable envelope it MUST return state plus the validated result (GetObject only when `payload_pointer` is non-null); for completed with missing/null pointer, missing object, or corrupt JSON it MUST return `{ state: "Completed" }` without `result`; for a failed request it MUST return state plus the terminal error code and no content; for cancelled or `AwaitingContext` it MUST return state only; for in-flight states it MUST return `{ state, pending: true }`; for an unknown reference or installation mismatch it MUST return not found; missing/invalid token → 401 `unauthenticated`. (§5.5, §7.6, §7.4, §7.3)
- **FR-011**: The get-request endpoint MUST use exactly one indexed D1 lookup on `request_reference` and MUST NOT perform a second D1 lookup or a table scan; failed, cancelled, `AwaitingContext`, and in-flight requests MUST perform no R2 read; GetObject MUST NOT use a derived-key fallback when `payload_pointer` is null. (§7.6, §7.3)
...
- **Get-request response** (§5.5, §7.6): `{ state, terminal_error_code?, result?, pending? }` — state always present; `terminal_error_code` present only for `failed`; `result` present only for `completed` when the envelope is readable; `completed` without envelope → state only; `cancelled` / `AwaitingContext` → state only; in-flight → `{ state, pending: true }`; unknown / installation mismatch → not found; missing/invalid AAT → 401 `unauthenticated`. Established here for the client-facing read path (distinct from F3's operator-only support lookup, which returns the full trace plus the whole envelope). Existing Completed-with-result / Failed / Cancelled success meanings are unchanged (delivery plan §2.3).
...
- **Data Integrity & Security**: The journal row is the auditable life of a request — submission, principal, capability and prompt artifact versions, state timeline, terminal error code, and payload pointers (§4.3.11, §7.3). The row is created before work begins so it cannot be lost to a later failure, and a guard rejection produces no row (rejections are counted by rate-limit/admission, not journaled) so the cheap-rejection path stays cheap (§6.2, §7.5). The R2 envelope carries prompt, context, raw responses, and result; its retention class is a per-capability manifest field (§5.1, §7.7), expired by F3 — C3 writes it but does not purge it. The get-request endpoint is AAT-authenticated and installation-scoped; it exposes only journaled state and the validated result (when available) to the caller; operator-only full-trace support lookup is F3, not C3 (§5.5).
- **Failure Handling**: A stage-9 insert failure fails the request before any paid work (`internal_error` with `request_reference` and `trace_id`); a stage-16 failure never fails the already-completed request (logged via `console.error`), because the request's existence is durable from stage 9 and stage 16 carries only diagnostic detail (§6.1, §4.3.11). The get-request endpoint degrades to not found for an unknown reference / installation mismatch, to state-without-result for completed-without-envelope, and to 401 for missing/invalid AAT; it does not synthesize a result. Quota DO unavailability and provider failures are handled by B4 and D3, not C3.
...
- **SC-002**: An automated test proves a guard-rejected request produces zero `ai_request` inserts (spy) and is counted in `platform_counter` via the rate-limit/admission live path — the "no row on rejection" invariant (journal tally helpers are not part of the surface).
- **SC-003**: An automated test proves every §6.3 state transition is journaled with a timestamp, so the state timeline is reconstructable from the row (§8.9), and that terminal states are immutable (second write no-op).
...
- **SC-007**: An automated test proves the authenticated get-request endpoint returns, via exactly one indexed D1 lookup on `request_reference`: completed-with-envelope → state plus validated result; completed-without-envelope → state only; failed → state plus terminal error code and no content; cancelled / `AwaitingContext` → state only; in-flight → `{ state, pending: true }`; unknown / installation mismatch → not found; missing/invalid AAT → 401 `unauthenticated` — and that failed/cancelled/`AwaitingContext`/in-flight perform no R2 read.
