...
C3 makes the journal the audit backbone of the gateway: it creates the durable `ai_request` row synchronously at stage 9 before any inference work begins, stamps every §6.3 state transition (each transition overwrites `state` and stamps `updated_at`; the terminal one also stamps `completed_at` — no per-transition history table, per the §6.3 amendment), updates the row with its terminal state at stage 15, and writes the per-attempt `ai_attempt` rows, exactly one `usage_event` ledger row, and exactly one R2 payload envelope (JSON, keyed `request/{id}/envelope`, four sections `context`/`prompt`/`attempts[]`/`result`) in a `ctx.waitUntil` post-response continuation at stage 16 whose failure never fails the already-completed request. A guard rejection produces no row. Through the get-request endpoint, a request reference resolves — through one indexed D1 lookup on `request_reference` — to the terminal state and, for a completed request only, the validated result fetched from the envelope's `result` section. The slice sits in band C, after A5 froze the D1 schema and C1 froze the resolved manifest, and before D3/D4/D6 feed the writer per-attempt and validated-result data and F3 consumes the frozen journal and envelope (delivery plan §3.4, row C3).
...
**Constraints**: Stages 9/15/16 emit only `internal_error` (stage 9, on D1 insert failure); stage 16 emits no error to the client by construction (§6.1). Terminal error codes written to the row are produced by D3/D4/D6 and recorded, not generated, by C3. The envelope is JSON (Clarification Q1), one object per request (§7.4.1). Stage 16 runs via `ctx.waitUntil` and swallows failures (Clarification Q3). No new D1 entity or column (the §6.3 amendment confirms the three milestone timestamps suffice; the schema is unchanged). No mechanism from §9.14 (R-20).
...
- [x] Security remains authenticated, tenant-scoped, branch-scoped, permission-gated,
      auditable, and soft-delete-preserving — C3 consumes the immutable `Principal` (B3) and the request reference (A6) as read-only arguments; it performs no authentication, holds no secrets, and writes no clinic audit row. The journal row is the platform's own audit of a request's life; a guard rejection produces no row (§6.2), so refusing abuse stays cheap. The get-request endpoint exposes only the terminal state and the validated result to the caller — operator-only full-trace support lookup is F3, not C3 (§5.5).
...
| §5.5 API surface — Get request | **Created** — the `getRequest(reference, bindings)` read function and its `GET /v1/requests/{reference}` route in `worker.ts`. One indexed D1 lookup on `request_reference`; for `completed` additionally one R2 `GetObject` for the `result` section; `failed` returns state + terminal error code, no R2 read; `cancelled` returns state only, no R2 read; unknown returns not found. | Yes — the get-request endpoint is a standalone §5.5 read surface C3 freezes (distinct from F3's operator-only support lookup). |
...
| --- | --- | --- |
| `ai-platform/src/journal/index.ts` | Created | FR-001, FR-002, FR-003, FR-004, FR-005, FR-006, FR-007, FR-008, FR-009, FR-012 — `RequestRowInput` and `createRequestRow(input, db): {ok:true} | {ok:false, code:"internal_error"}` (stage 9, FR-001/FR-002); `TransitionState` (the §6.3 states) and `journalTransition(requestId, state, now, db)` (stamps `state`+`updated_at`; terminal also stamps `completed_at`, FR-003); `recordTerminalState(requestId, state, terminalErrorCode?, now, db)` (stage 15, FR-004/FR-006); `AttemptInput`, `PostResponseInput`, `Envelope`, and `writePostResponseDetail(input, {db, r2, ctx})` scheduling the stage-16 writes via `ctx.waitUntil` (N `ai_attempt` inserts + one `usage_event` insert + one R2 `PutObject` keyed `request/{id}/envelope` + `ai_request.payload_pointer` update), with the inlined `buildEnvelope(input): string` JSON helper (four sections `context`/`prompt`/`attempts[]`/`result`, FR-007/FR-008/FR-009/FR-012); `GetRequestResult` and `getRequest(reference, {db, r2}): GetRequestResult` (one indexed D1 lookup on `request_reference`; `completed`→state+result via one R2 `GetObject`; `failed`→state+terminal_error_code, no R2 read; `cancelled`→state only; unknown→not found, FR-010/FR-011). |
| `ai-platform/src/worker.ts` | Modified | FR-010, FR-011 — adds `GET /v1/requests/{reference}` route: normalise the reference (A2 `normalizeRequestReference`), call `getRequest`, map `GetRequestResult` to a JSON `Response` (`completed`→200, `failed`→200 with body, `cancelled`→200 with body, unknown→404). The POST submit stream (A6) and control routes are unchanged. |
...
| `guard_rejected_produces_no_row` | T-C3-05 | Pipeline tests | workers | FR-005 / SC-002 — a guard-rejected request produces zero `ai_request` inserts (spy on D1 inserts: 0) and one `platform_counter` increment. |
| `every_state_transition_timestamped` | T-C3-06 | Pipeline tests | workers | FR-003 / SC-003 — each §6.3 transition (Accepted, Composing, Invoking, Streaming, Validating, Repairing, AwaitingContext, Completed, Failed, Cancelled) stamps `state`+`updated_at` (terminal also stamps `completed_at`); the milestones + deterministic §6.3 graph recover the timeline. (Parameterised: one case per transition.) |
...
| `get_request_completed_returns_state_and_result` | T-C3-14 | Pipeline tests | workers | FR-010 / SC-007 — a completed request's reference returns state plus the validated result, via one indexed D1 lookup + one R2 `GetObject`. |
| `get_request_failed_returns_state_and_error_no_content` | T-C3-15 | Pipeline tests | workers | FR-010 / SC-007 — a failed request's reference returns state plus the terminal error code and no content (no R2 read). |
| `get_request_cancelled_returns_state_only` | T-C3-16 | Pipeline tests | workers | FR-010 / SC-007 — a cancelled request's reference returns state only (no R2 read). |
| `get_request_unknown_reference_returns_not_found` | T-C3-17 | Pipeline tests | workers | FR-010 / SC-007 — an unknown reference returns not found, via the indexed lookup. |
| `get_request_uses_exactly_one_indexed_query` | T-C3-18 | Pipeline tests | workers | FR-011 / SC-007 — the get-request path performs exactly one indexed D1 lookup on `request_reference` (spy on D1 queries: 1); failed/cancelled perform no R2 read. |
