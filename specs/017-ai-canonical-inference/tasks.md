...
- [X] T009 [US1] In `ai-platform/src/contracts/canonical.ts`, define the four canonical element interfaces (`CanonicalRequest`, `CanonicalStreamChunk`, `CanonicalResult`, `CanonicalError`) with **real per-field TypeScript types** (not `unknown`), keyed by the T001 manifest names; type `CanonicalError["taxonomy code"]` as the `TaxonomyCode` imported from `ai-platform/src/errors.ts` (A2's frozen set, consumed not redefined); export `CANONICAL_MESSAGE_ROLES`. Adapters/composer consume typed fields without `as` casts (T-A3-09). Satisfies FR-001, FR-003, FR-004, FR-006, FR-007; unblocks T005..T008.
