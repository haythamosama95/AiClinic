...
**Constraints**: A manifest is immutable per version and the manifest handed downstream or to a discovery caller must not be mutable by that caller (§5.1) — deep-frozen at the registry boundary (unmodifiable Map); `resolve` may return a derived frozen copy when overlay changes Identity. Only an exact requested version resolves; the resolver honours the pin rather than picking a "compatible" version (§4.3.4). Plan-level allowances and grant-version match are enforced in `resolve()` (`forbidden_capability`). A guard rejection at stage 5 produces no `ai_request` row (§7.5, §6.2) — that invariant is C3's and is consumed here, not re-built. No per-request server-side state (§4.4, §9.7). No deprecation overlap window — that is J1; C1 recognises effective `deprecated`, serves it on resolve, and may advertise it in discovery with successor identity when overlay/published provides it. Kill switches are not applied to discovery. No second Quota DO round trip and no R2 object — C1 performs neither.
...
| **B3 / A5** — in-isolate config-cache surface for kill-switch scopes and entitlement (§4.3.4, §6.1 stage 5) | `ai-platform/src/config-cache/index.ts` — `ConfigCache`, `loadConfig(cache, reader, kind, key)`, `D1Reader`, `ConfigCacheMissError`, `ConfigEntityKind` (`"kill_switches"`, `"entitlements"`, `"grants"`). Frozen artifact: `specs/019-ai-context-keys-d1-config/contracts/config-cache.md`. | C1 calls `loadConfig("kill_switches", …)` for the four kill-switch scopes at resolve time (FR-005; miss ⇒ inactive) and `loadConfig("entitlements", …)` / `loadConfig("grants", …)` for both resolve allowance/grant checks (FR-002 → `forbidden_capability`) and discovery filtering (FR-012). It consumes the **surface**; it does **not** import or call B3's `evaluateEntitlement` (stage 3 still owns that path; resolve fails closed independently). The warm-isolate no-D1-read invariant is A5/B3's and is not re-proven by C1. |
| **A6** — protocol adapter's parsing of the version-pin header and normative HTTP-status mapping (§5.4) | `ai-platform/src/adapter.ts` — `AdapterStreamContext.headers.capabilityVersion`. `ai-platform/src/errors.ts` — `liveHttpStatusForCode`. | C1 emits the four taxonomy codes (`capability_unknown`, `capability_retired`, `forbidden_capability`, `capability_disabled`); it does not re-implement their HTTP translation, which stays the adapter's (A6 / A2 `liveHttpStatusForCode`). C1 is agnostic to the header-parse shape — it receives the requested version as a string argument. No adapter file is modified. |
...
| --- | --- | --- |
| `ai-platform/src/capability/index.ts` | Created | FR-001, FR-002, FR-003, FR-004, FR-005, FR-006, FR-007, FR-008, FR-009, FR-010, FR-011, FR-012, FR-013, FR-014 — `CapabilityRegistry` (the `${capabilityId}@${version}` → immutable `Manifest` map built from bundled manifests via A4 `load()`; unmodifiable Map; install-once `setCapabilityRegistry`), `resolve(…)` returning `{ok, manifest}` or `{ok:false, code}` including `forbidden_capability` for allowance/grant failures, `discover(…)` returning granted effective-active/deprecated `Manifest[]` plus an etag over the filtered overlay-derived set, `buildDiscoveryResponse(…)` returning `200`/`304` with quoted `ETag` and `Cache-Control: private, must-revalidate` (A4 `hashManifest` SHA-256). |
...
| `discovery_only_granted_active` | T-C1-07 | Pipeline tests | `vitest.workers.config.ts` | FR-011, FR-012 / SC-005 — discovery for a principal seeded with an `entitlements.allowed_capabilities` list and `grants` rows returns only granted manifests whose effective lifecycle is `active` or `deprecated`; effectively `retired` and un-granted ones are absent. |
| `discovery_etag_not_modified` | T-C1-08 | Contract tests | `vitest.config.ts` | FR-013 / SC-006 — `buildDiscoveryResponse()` with a matching `If-None-Match` (quoted strong tag / weak comparison) returns `304`; the manifest list is not re-serialised; `Cache-Control: private, must-revalidate` on 200 and 304. |
| `discovery_etag_changes` | T-C1-09 | Contract tests | `vitest.config.ts` | FR-013 / SC-006 — changing one fixture manifest in the filtered discovery set changes the computed etag (A4 `hashManifest` SHA-256 over overlay-derived manifests). |
...
Coverage additions from §3.10: every error code the slice can emit is covered one-for-one — `capability_unknown` (T-C1-02), `capability_retired` (T-C1-03), `capability_disabled` (T-C1-04), plus `forbidden_capability` on resolve for allowance/grant failures (FR-002 / FR-010; B3 still owns stage 3); the version-pin boundary (T-C1-01, the different-version half); the deprecation-vs-retirement branch (T-C1-05 vs T-C1-03); the no-`ai_request`-row-on-guard-rejection prohibition is C3's invariant and is consumed, not re-asserted, by C1 (spec Edge Cases). An ineligible capability is filtered out of discovery (T-C1-10) rather than emitted as an error.
