...
**Prerequisites**: `plan.md` (required), `spec.md` (required for the user story). `research.md` is never produced on this platform (the research is `docs/architecture/17-ai-platform.md`). `data-model.md` is not produced (C1 defines no D1 entities). `contracts/` and `quickstart.md` are written during Phase 5.
...
- [ ] T002 [US1] Create `ai-platform/test/capability.test.ts` with the inline-fixture substrate: a `validManifest(id, version, lifecycleState?)` factory producing a manifest declaring all ten §5.1 field groups (reusing A4's `validManifest()` shape from `test/manifest.test.ts`), an in-memory `buildRegistry(...manifests)` helper returning the `CapabilityRegistry` value under test, a `buildPrincipal({ installationId, plan, allowedCapabilities })` fixture producing a B3 `Principal` (`Object.freeze`d), and the import of `resolve`, `discover`, `buildDiscoveryResponse`, and the `CapabilityRegistry` type from `../src/capability`. File fails to compile (module absent). Proves the fixture substrate for every later test.
- [ ] T003 [US1] Add `T-C1-01 resolver_exact_pin_resolves` to `ai-platform/test/capability.test.ts` (unit, `vitest.config.ts`): register a fixture manifest at `clinic.test@1.0.0`; `resolve(principal, "clinic.test", "1.0.0", cache, reader)` returns `{ok:true, manifest}` whose `Identity.version === "1.0.0"`; a request for a different version (e.g. `"1.2.0"`) on the same capability id returns `{ok:false, code:"capability_unknown"}` — the pin is honoured, not relaxed to a "compatible" version (§4.3.4). Satisfies FR-001, FR-008 / SC-001.
- [ ] T004 [US1] Add `T-C1-02 resolver_unknown_capability` to `ai-platform/test/capability.test.ts` (unit): `resolve()` for a capability id not present in the registry returns `{ok:false, code:"capability_unknown"}` (§6.1 stage 5; §5.4). Satisfies FR-003, FR-010 / SC-002.
- [ ] T005 [US1] Add `T-C1-03 resolver_retired_rejected` to `ai-platform/test/capability.test.ts` (unit): a fixture manifest with `Identity.lifecycleState === "retired"` makes `resolve()` return `{ok:false, code:"capability_retired"}` (§4.3.4; §5.1). Satisfies FR-004, FR-010 / SC-002.
- [ ] T006 [US1] Add `T-C1-04 resolver_killed_capability_disabled` to `ai-platform/test/capability.test.ts` (integration, `vitest.workers.config.ts`): seed `env.DB` with the A5 migration, insert an active `kill_switches` row for the `capability:<id>` scope, and assert `resolve()` returns `{ok:false, code:"capability_disabled"}` — read through `loadConfig(cache, reader, "kill_switches", …)` against the real D1 (§4.3.4; §6.1 stage 5; §5.4). Satisfies FR-005, FR-006, FR-010 / SC-002.
- [ ] T007 [US1] Add `T-C1-05 resolver_deprecated_serves` to `ai-platform/test/capability.test.ts` (unit): a fixture manifest with `Identity.lifecycleState === "deprecated"` is **not** rejected; `resolve()` returns its `Manifest` (§4.3.4; §5.1 — deprecation is not in the rejection list; the overlap window is J1, out of scope). Satisfies FR-007 / SC-003.
- [ ] T008 [US1] Add `T-C1-06 resolver_manifest_immutable` to `ai-platform/test/capability.test.ts` (contract, `vitest.config.ts`): the `Manifest` returned by `resolve()` cannot be mutated by the caller — attempt to set a field on `manifest.Identity` and to reassign `manifest["Context requirements"]`; assert each attempt no-ops or throws and a subsequent reader sees the original values (A4's `Proxy` freeze holds, §5.1). Satisfies FR-008 / SC-004.
- [ ] T009 [US1] Add `T-C1-07 discovery_only_granted_active` to `ai-platform/test/capability.test.ts` (integration, `vitest.workers.config.ts`): seed `env.DB` with an `entitlements` row whose `allowed_capabilities` includes one capability id and a `grants` row for it; register fixture manifests with `active`, `deprecated`, and `retired` lifecycle states plus one un-granted active one. Assert `discover(principal, cache, reader)` returns only the granted, `active`-lifecycle manifest (§5.5; §4.3.4). Satisfies FR-011, FR-012 / SC-005.
- [ ] T010 [US1] Add `T-C1-08 discovery_etag_not_modified` to `ai-platform/test/capability.test.ts` (contract, `vitest.config.ts`): `buildDiscoveryResponse(request, manifestList, etag)` with `request.headers.get("If-None-Match")` equal to the computed etag returns a `304` response and does not re-serialise the manifest list; a mismatched `If-None-Match` returns `200` with the list (§5.5; §5.2). Satisfies FR-013 / SC-006.
- [ ] T011 [US1] Add `T-C1-09 discovery_etag_changes` to `ai-platform/test/capability.test.ts` (contract, `vitest.config.ts`): compute the etag for a granted+active fixture set, then swap one fixture manifest's `Identity.version` (or remove one from the granted set) and recompute; assert the two etags differ (the hash is over the manifest content, reusing A4's `hashManifest` for stability, §5.5; §5.2). Satisfies FR-013 / SC-006.
- [ ] T012 [US1] Add `T-C1-10 discovery_entitlement_gated_absent` to `ai-platform/test/capability.test.ts` (integration, `vitest.workers.config.ts`): seed `env.DB` with an `entitlements` row whose `allowed_capabilities` omits a capability id (or whose `plan` is below the manifest's `Access.minimumPlanTier`), and assert that capability is absent from the `discover()` result for that installation — filtered out, not emitted as an error (§4.3.4; §5.5; spec Edge Cases — `forbidden_capability` is B3's stage-3 code, not C1's). Satisfies FR-002, FR-012 / SC-005.
...
- [ ] T013 [US1] Create `ai-platform/src/capability/index.ts` — the `CapabilityRegistry` (`Map<"${capabilityId}@${version}", Manifest>` built from manifests passed through A4's `load()`, so the never-names-provider/model rule and immutability are inherited, not re-implemented), `resolve(principal, capabilityId, version, cache, reader)` returning `{ok:true, manifest}` or `{ok:false, code}` where `code` is `"capability_unknown"` (id+version not registered), `"capability_retired"` (`Identity.lifecycleState === "retired"`), or `"capability_disabled"` (any of the four kill-switch scopes active, read via `loadConfig(cache, reader, "kill_switches", …)` — `global`, `capability:<id>`, `installation:<id>`, `provider:<id>`). Honours the exact pin (no "compatible" version fallback). Serves `deprecated` unchanged. Returns the A4 `Manifest` (already `Proxy`-frozen), so FR-008 is inherited. **Satisfies**: FR-001, FR-002, FR-003, FR-004, FR-005, FR-006, FR-007, FR-008, FR-009, FR-010; **proved by**: `T-C1-01`, `T-C1-02`, `T-C1-03`, `T-C1-04`, `T-C1-05`, `T-C1-06`.
- [ ] T014 [US1] Add `discover(principal, cache, reader)`, `buildDiscoveryResponse(request, manifestList, etag)`, and the etag helper to `ai-platform/src/capability/index.ts` — `discover()` enumerates the registry, filters to `Identity.lifecycleState === "active"` (§5.5 "active manifests"; `deprecated`/`retired` excluded from discovery though `deprecated` is served by `resolve()`), filters by the principal's installation via `loadConfig(cache, reader, "entitlements", principal.installationId)` (`allowed_capabilities`) and `loadConfig("grants", "${installationId}/${capabilityId}")` (not revoked), and by `Access.minimumPlanTier` against the entitlement `plan`; returns the resulting `Manifest[]` plus an etag computed as `hashManifest` over the canonical serialisation of the granted+active set (reusing A4's `hashManifest` for stability — no new hashing mechanism, R-20). `buildDiscoveryResponse()` returns `304` when `If-None-Match` equals the etag and `200` with the JSON body otherwise (§5.5; §5.2). **Satisfies**: FR-011, FR-012, FR-013, FR-014; **proved by**: `T-C1-07`, `T-C1-08`, `T-C1-09`, `T-C1-10`.
