...
**Prerequisites**: `plan.md` (required), `spec.md` (required for the user story). `research.md` is never produced on this platform (the research is `docs/architecture/ai-platform/01-ai-platform.md`). `data-model.md` is not produced — J3 defines no new D1 entity (spec Key Entities). `contracts/staged-rollout-canary.md` is already frozen on disk (written during the plan phase per DP-4; `AVAILABLE_DOCS` includes `contracts/`). `quickstart.md` is written in Phase 5 (Documentation).
