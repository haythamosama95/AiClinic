...
- [ ] T024 [US1] Add `ALREADY_ENROLLED` guard to `auth_internal.enroll_installation_keypair()` in `backend/supabase/migrations/20260801120100_ai_installation_keypair_routines.sql` (reject when any active key exists: `is_deleted = false AND revoked_at IS NULL`; permit re-enroll when all keys revoked) and add keystore suite `T11 enroll with active key → ALREADY_ENROLLED; re-enroll after all keys revoked succeeds` to `backend/tests/ai_keystore_rls.sql`. Satisfies FR-002c; pinned by T11.
