...
through the provider port. Per-target retries are bounded by that target's `max_attempts`, backoff
includes jitter, and a retry occurs only when the adapter classifies the failure as retryable.
After retryable exhaustion (including timeout), the loop falls back to the next chain entry. When
every candidate is exhausted under retryable failure, the request fails with `provider_unavailable`.
If partial streamed text was already produced for a failed target, fallback emits an explicit
`regenerating` signal, discards that text, and never splices two providers' output (§4.3.7; §8.6;
§6.6).
...
| Invoke path | Each attempt calls the provider port for that target's `provider_id` with the same request identity (no new user-initiated request / idempotency key) (§6.6; FR-009) |
| Retry bound | At most `max_attempts` invokes of a given chain entry (FR-001, FR-010) |
...
| Backoff | Between retries of the same target, delay is bounded and **jittered** — requested delays across retries are not all identical (FR-002; Clarification Q3: injectable sleeper) |
| Timeout | Timeouts are retryable in the §8.6 sequence; exhausting a target via timeouts advances with `fallback_after_timeout` (FR-004) |
| Exhausted chain | When every chain entry has failed retryably to its bound (or the chain is empty), fail with taxonomy code `provider_unavailable` (FR-005). No new taxonomy code is invented |
...
| Event / attempt sink | Collects attempt records and stream signals for journaling / later stream wiring (Clarification Q4) |
...
| `fallback_after_retryable_error` | First attempt on a subsequent target after prior target exhausted via non-timeout retryable failures |
| `fallback_after_timeout` | First attempt on a subsequent target after prior target exhausted via timeout-classified retryable failures |
...
| Rule | Detail |
| --- | --- |
| Trigger | Fallback to a new chain entry **after** the failed target had already produced streamed text visible to the sink |
| Signal | Emit an explicit `regenerating` event on the sink before the fallback target's output is considered |
...
T6 drives a **test-only** port double/harness that yields partial chunks then a retryable error.
D2's production `FakeAdapter` and its scripted-outcome set are **unchanged**. The harness lives with
the invocation suite and is not a new production adapter.
...
| `outcome` | `success` \| `retryable_failure` \| `terminal_failure` \| `timeout` |
| `error_code` | Taxonomy code when the attempt failed; absent on success |
| `request_id` / request identity | Same request across internal retries (FR-009; T12) |

Optional timing/usage fields may be present when the port returns them; they are not new Freezes
beyond feeding C3's existing `AttemptInput` / envelope `attempts[]` shape when production wiring
lands. `selection_reason` travels on this feed; persisting it into a D1 column is outside this slice
(spec: no new D1 columns; C3 write-path unchanged).
...
| T3 | Backoff delays not all identical (jitter) |
| T4 | One journaled attempt per invoke |
| T5 | Exhausted chain → `provider_unavailable` |
| T6 | Partial stream then fallback → `regenerating`; earlier text discarded; no splice |
| T7 | Per-target `max_attempts` / chain budget never exceeded |
| T8 | Primary success; single attempt; `selection_reason = primary` |
| T9 | Timeout exhaustion → `fallback_after_timeout` |
| T10 | Fallback walks only the given chain |
| T11 | No provider-history / circuit-breaker consultation (spy) |
| T12 | Internal retries keep the same request identity |
