...
**Prerequisites**: `plan.md` (required), `spec.md` (required for the user story). `research.md` is never produced on this platform (the research is `docs/architecture/17-ai-platform.md`). `data-model.md` is omitted — H2 defines no D1 entity (spec Key Entities; FR-019). The three `contracts/*.md` artifacts are already frozen on disk (written during the plan phase). `quickstart.md` is named in the plan and filled in Phase 4 (Documentation).
