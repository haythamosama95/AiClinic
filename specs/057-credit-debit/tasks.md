...
- [X] T016 [US1] Modify `ai-platform/test/soft-threshold-routing.test.ts` (fixture only) — apply G1 `plan_catalogue` migration so `credit_budget` exists; seed `credit_budget` / `creditsUsed` (via `seedEntitlement` + the admit/credit helper passing `credits`) so F4 routing cases still cross the `degraded` flag G2 now sets from the credit ratio. Do not change `routing_tier` / `degraded_notice` assertions. Do not rewrite `ai-platform/src/soft-threshold/`. Depends on T011–T013. **Satisfies**: FR-009 (do not rewrite F4 routing). **Proved by**: F4 routing cases remaining green under Verification (G2 `credit_ratio_soft_threshold_sets_degraded_flag` is the credit-ratio behaviour).
  ...
  Fixture-only prior-suite amendments (same strategy; §3.10 scope is the full workers pool — see plan Test Layout). No assertion weakening, no `.skip`, no production change:
  - [X] T016a [US1] `ai-platform/test/system/harness.ts` — `EntitlePayload` gains optional `credit_budget`; `seedCataloguePlan` binds `payload.credit_budget ?? 10_000` (G1 live-plan default) instead of the leftover `0` that G2 remaining-budget treats as immediately exhausted. Covers every `test/system/**/*.system.test.ts` admitted path (golden-journey, lifecycle-interplay, entitlement-grant-interplay, routing-policy-traffic, settlement-integrity, token-contract-rotation, capability-lifecycle, cron-retention-interplay, failure-taxonomy-matrix, quota-admission-interplay).
  - [X] T016b [US1] `ai-platform/test/entitle-grant.test.ts` — `seedCataloguePlan` binds the G1 live-plan default `10_000` instead of `0`.
  - [X] T016c [US1] `ai-platform/test/pipeline.test.ts` — apply G1 `plan_catalogue` migration in `beforeAll`; entitlements reader SELECT and `seedEntitlement` gain `credit_budget` (positive default 10_000 + `creditBudget` override); `pipeline_soft_threshold_exposes_degraded_routing_tier` seeds `creditBudget: 2` and settles `credits: 1` so the second guard crosses the 0.5 credit ratio. `routingTier` / `degraded` assertions unchanged.
  - [X] T016d [US1] `ai-platform/test/worker-request-orchestrator.test.ts` — apply G1 `plan_catalogue` migration in `beforeAll`; `seedInstallationFixture` entitlement INSERT gains `credit_budget: 10_000`; `seedSoftThresholdTieredFixture` sets `credit_budget = 2` so one settled credit (quotaWeight 1) crosses the 0.5 credit ratio. `degraded_notice` / `routing_tier` / provider-failure assertions unchanged.
  - [X] T016e [US1] `ai-platform/test/system/quota-admission-interplay.system.test.ts` — SYS-5.2 seeds `credit_budget: 2` (credit-ratio degraded on second invoke) and its phase-three UPDATE raises `credit_budget = 100` alongside `request_quota`; SYS-5.7 seeds credit ceilings (`credit_budget: 1` admit-then-exhaust; `credit_budget: 0` immediately exhausted) because token/cost counters are settlement-only under G2 (FR-015). All status / `quota_exhausted` / `routing_tier` assertions unchanged.
