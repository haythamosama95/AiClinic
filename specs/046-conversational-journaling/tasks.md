...
**Prerequisites**: `plan.md` (required), `spec.md` (required for the user story). `research.md` is never produced on this platform (the research is `docs/architecture/ai-platform/01-ai-platform.md`). `data-model.md` is omitted — H3 defines no D1 entity; columns were reserved in A5 (spec Key Entities; FR-002, FR-004). `contracts/conversational-journaling.md` is already frozen on disk (written during the plan phase). `quickstart.md` is named in the plan and filled in Phase 4 (Documentation).
