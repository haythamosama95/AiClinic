...
**Prerequisites**: `plan.md` (required), `spec.md` (required for the user story). `research.md` is never produced on this platform (the research is `docs/architecture/ai-platform/01-ai-platform.md`). `data-model.md` is not produced — E4 defines no D1 entities (spec Key Entities: not applicable). `contracts/ai-availability-flag.md` is named in the plan Files section and confirmed in Phase 4 (Documentation). `quickstart.md` is written in Phase 4.
...
- [X] T018 [US1] Create `frontend/lib/features/ai/host/ai_feature_host_page.dart` and modify `frontend/lib/app/app_routes.dart` + `frontend/lib/app/router.dart` — standalone host composing availability gate + surface for widget tests and CP3 entry; register route constant; route builder hosts `AiFeatureHostPage` when `AiFeatureHostDependencies` are supplied via GoRouter extra (Clarification Q2; not embedded in a production clinical screen; production mint/submit adapters are composition-time). Depends on T013–T014 and T017. **Satisfies**: FR-001, FR-009, FR-012. **Proved by**: T8–T10, T21 (host gate); surface suite via host composition as needed.
