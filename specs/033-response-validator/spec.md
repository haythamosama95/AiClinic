...
- Q: How should bounded repair re-ask (T12–T16) be invoked so D6 owns the attempt without rewriting D3’s invocation/retry/fallback loop? → A: Validator accepts an injected `reask(errors) => Promise<{ output; usage }>` port; tests script invalid-then-valid (or always-invalid) outcomes. The D6 structured broker (`stream/structured.ts`) exposes the same repair seam (`repairPolicy` + `reask` + attempt/cost sinks) and routes commit-time completion through `validateAndRepair`. Wiring `reask` to D3 invocation may land later; the broker seam exists in this slice so a `repairPolicy.allowed: true` capability is repair-capable on the D6-owned path `[implementation choice — no §citation]`
...
16. **Given** a request that performs a repair re-ask, **When** usage is settled, **Then** the usage reported by the `reask` call is counted against the request via the repair-cost sink (not a caller-supplied constant unrelated to that generation). *(repair cost is counted against the request)*
...
- **FR-006**: Repair cost MUST be counted against the request from usage returned by the `reask` port (tokens/cost of the repair generation). Callers MUST NOT satisfy this by injecting a fixed per-attempt constant in place of re-ask usage (§4.3.9; delivery plan §3.5 Done when).
...
- **FR-015**: When `outputSchemaRef` is non-null and no schema runner is registered for that ref, validation MUST fail at the schema phase. When a declared `businessValidationRuleRefs` entry has no registered runner, validation MUST fail at the business-constraint phase. Declared refs MUST NOT be silently skipped (fail closed).
- **FR-016**: A terminal `validation_failed` result from `validateAndRepair` MUST include the failing phase and a human-readable failure message (from the phase `ValidationError`) so journaling/support can record why validation failed without re-deriving it.
...
- **H1 / H2 (conversational manifest / transcript validation)**: Conversational interaction mode, context-request as a second permitted output shape, and conversation budgets remain H1/H2. D6 owns the ordered phase module (`validate/phases.ts`) and may host transport/parse helpers H2 consumes (e.g. conversational context-request parse). H2 must not reorder/skip phases or fork a second validator; conversational product rules and budgets stay out of D6’s Freezes.
