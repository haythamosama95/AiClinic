...
**Primary Dependencies**: Existing `ai-platform/` Worker tree — `@cloudflare/vitest-pool-workers` / Miniflare with real D1, R2, and Quota DO (`DO` → `GatewayObject`) bindings via `vitest.workers.config.ts` and Wrangler development env; Vitest ~3.2. Counting spies wrap those bindings (Clarification Q1). Happy-path under load drives admission + credit + one R2 envelope through the existing pipeline modules with D2's **fake** provider (Clarification Q2) — no live provider egress. No new external package.
...
**Testing**: Vitest workers-pool (`npx vitest run --config vitest.workers.config.ts`). Named tests T1–T9 are the §13.5 **Load and cost tests** layer / delivery plan §3.11.6 row F5. Dedicated npm script `test:load` is the checkpoint gate (Clarification Q4; FR-002). Concurrency fixture `N=20`; assert guard p95 `< 100` ms (Clarification Q3 — suite encoding of "target concurrency" / "tens of milliseconds", without rewriting FR/SC prose).
...
**Project Type**: Additive, non-primary AI gateway load/cost suite. Per the §14 acknowledgement: the Worker holds no domain logic, no business data, and no write path into Supabase. F5 adds no runtime pipeline stage.

**Performance Goals**: Guard p95 under the concurrency fixture stays within tens of milliseconds (FR-003; suite fixture `p95 < 100` ms at `N=20` — Clarification Q3). Under load, preserve platform I/O budgets: one R2 Class A and two DO trips per request (§13.6; §13.6.1; delivery plan §6.4). D1 headroom and DO throughput are measured, not ceiling-gated here (FR-006, FR-007).

**Constraints**: Workers-pool Miniflare + binding spies only (Clarification Q1). Full happy path with fake provider (Clarification Q2). No rewrite of D7 adapter / fixtures / policy registration (FR-010; Consumes D7). No invented numeric ceilings for D1 headroom or DO throughput (FR-006, FR-007; Edge Cases). No §5.4 taxonomy codes from this suite. No per-request server-side state (T8; §4.4, §9.7). No Flutter prompt/provider/model strings (T9; R-12). No second R2 object or third DO trip (T6, T7; §7.5, §13.6). No §9.14 mechanism (R-20). Pre-flight estimator remains C2 (Out of Scope).

**Scale/Scope**: One load/cost suite under `ai-platform/test/load/`. Zero §4 runtime components modified (see Components Touched). Nine named tests (T1–T9). One Freezes contract. Roughly 14–20 tasks — under the ~25-task ceiling (delivery plan §6.3 / skill stop condition 5).
...
**§14 acknowledgement (gateway slice):** The Worker is an additive, non-primary component with no domain logic, no business data, and no write path into Supabase. F5 adds no store, no request-path I/O of its own beyond exercising existing bindings under test, and no per-request state; the load and cost suite is workers-pool tooling under `ai-platform/test/load/` that proves the §13.6.1 metered footprint and CP5 measurement gate.
...
2. **What was implemented** — workers-pool load suite, binding spies, fake-provider happy path under concurrency, structured measurement report, `test:load` checkpoint gate, frozen contract.
3. **Files to review** — this slice's `ai-platform/test/load/` files, `package.json` / vitest config deltas, and frozen contract only.
...
5. **Inspect the changes** — open the load entry, measurement-report shape, binding spies, and frozen contract.
...
`contracts/load-and-cost-tests.md` freezes the three Freezes entries so later checkpoints bind to a frozen artifact, not prose: the load and cost test layer (placement, timing, `test:load` gate), the under-load one-R2 / two-DO metered-footprint assertion, the CP5 measurement gate, and the structured in-test measurement report (finite values; no D1/DO ceilings; suite fixture `N=20` / `p95 < 100` ms).
...
└── test/
    └── load/                                 # NEW — load/cost suite root only (no src/load/)
        ├── binding-spies.ts                  # NEW — counting spies on D1 / R2 / DO bindings (Clarification Q1; FR-004–FR-008; T2/T3/T6/T7)
        ├── measurement-report.ts             # NEW — structured in-test report: finite values, no D1/DO ceilings (Clarification Q4; FR-006, FR-007; T4/T5)
        ├── happy-path.ts                     # NEW — drive full happy path under load with FakeAdapter: admission + credit + one R2 envelope (Clarification Q2; FR-001, FR-008)
        └── load-and-cost.test.ts             # NEW — T1–T9 (Load layer + inherited prohibitions)
```

No `frontend/` or `backend/` tree is shown — F5 touches neither. No `ai-platform/src/` module is added. D7 Gemini adapter / fixtures / policy data, B4 admission/credit / Quota DO, C3 journal / R2 envelope, and D2 `FakeAdapter` are **consumed or exercised unchanged** and are not listed as this slice's source tree.

**Structure Decision**: F5 extends the `ai-platform/test/` tree with a `load/` suite on the existing workers-pool Miniflare config (delivery plan §7.1; Clarification Q1), siblings to journal / admission / soft-threshold workers-pool tests. The Spec Kit template's `frontend/`/`backend/` conventions are deleted as unused, per the skill's repository-layout rule.
...
- T1: concurrency fixture `N=20`; assert guard p95 (stages 1–10 per §6.1) `< 100` ms (Clarification Q3).
- T2 / T6: spy counts R2 Class A (`PutObject` / equivalent) — exactly one per request; no second object.
- T3 / T7: spy counts Quota DO fetches — exactly two per request (admission + credit); no third trip.
- T4 / T5: structured measurement report carries finite `d1_hot_path_writes_per_request` and `do_throughput_per_installation` with **no** numeric ceilings (Clarification Q4).
- T8: asserts this suite introduces no per-request server-side state.
- T9: asserts this suite introduces no Flutter client files carrying prompt/provider/model strings.
...
2. **Binding spies + measurement report helpers.** Land `binding-spies.ts` and `measurement-report.ts` so T2–T7 / T4–T5 have metering and report shapes (Clarifications Q1, Q4; FR-004–FR-008).
3. **Happy-path driver under load.** Land `happy-path.ts` driving admission + credit + one R2 envelope with `FakeAdapter` on real Miniflare bindings (Clarification Q2; FR-001, FR-008).
4. **Load Vitest entry + T1–T5.** Concurrency fixture `N=20`, guard p95 `< 100` ms, one-R2 / two-DO assertions, measurement report for D1 headroom and DO throughput (Clarification Q3; FR-003–FR-007, FR-009).
5. **Inherited prohibitions + T6–T9.** Reinforce no second R2 / no third DO trip / no per-request state / no Flutter prompt-provider-model strings (delivery plan §6.4).
6. **Workers-pool wiring + `test:load`.** Include the load file in `vitest.workers.config.ts`, exclude it from the Node-pool config, and add `package.json` `"test:load"` as the checkpoint gate (Clarification Q4; FR-002; §3.10).
