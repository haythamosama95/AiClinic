...
### Database catalog import (`medication_catalog_names.json`)

For AiClinic dev seeding, `medication_catalog_names.json` contains only the `commercial_name_en` values formatted for the `medications.name` column (unique per organization, max 200 characters). The Flutter app bundles a copy at `frontend/assets/dev/egyptian_medication_names.json`.

Regenerate from the CSV:

```bash
python3 - <<'PY'
import csv, json
from pathlib import Path
src = Path('data/egyptian-medications/egyptian-drugs.csv')
seen, names = set(), []
with src.open(encoding='utf-8', newline='') as f:
    for row in csv.DictReader(f):
        name = (row.get('commercial_name_en') or '').strip()[:200]
        key = name.casefold()
        if name and key not in seen:
            seen.add(key)
            names.append(name)
payload = {'source': str(src), 'field': 'commercial_name_en', 'count': len(names), 'names': names}
for path in [
    Path('data/egyptian-medications/medication_catalog_names.json'),
    Path('frontend/assets/dev/egyptian_medication_names.json'),
]:
