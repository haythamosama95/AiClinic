...
void main() {
  group('appointmentCalendarFetchBounds', () {
    ...
    test('schedule mode returns Monday through next Monday', () {
      final dayStart = DateTime(focus.year, focus.month, focus.day);
      final weekStart = dayStart.subtract(Duration(days: dayStart.weekday - DateTime.monday));
      final (from, to) = appointmentCalendarFetchBounds(focus, AppointmentCalendarMode.schedule);
      expect(from, weekStart.toUtc());
      expect(to, weekStart.add(const Duration(days: 7)).toUtc());
    });
  ...
  group('appointmentCalendarNextFocus', () {
    ...
    test('schedule navigation advances one day', () {
      final next = appointmentCalendarNextFocus(DateTime(2026, 6, 15), AppointmentCalendarMode.schedule);
      expect(next, DateTime(2026, 6, 16));
    });
  });
}
