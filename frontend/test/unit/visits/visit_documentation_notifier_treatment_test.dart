...
void main() {
  ...
  group('refreshVisitPreservingDraft', () {
    ...
    test('merges attachments from get_visit without clearing SOAP draft', () async {
      final notifier = container.read(visitDocumentationProvider(visitId).notifier);
      await container.read(visitDocumentationProvider(visitId).future);

      notifier.updatePlan('Unsaved plan section');
      ...
      final state = container.read(visitDocumentationProvider(visitId)).value!;
      expect(state.plan, 'Unsaved plan section');
      expect(state.visit.attachments, hasLength(1));
      expect(state.visit.attachments.first.id, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa');
    });
  });

  group('family visit id', () {
    test('loads each family instance using ref family arg', () async {
      const otherVisitId = 'ffffffff-ffff-4fff-8fff-ffffffffffff';

      final primaryState = await container.read(visitDocumentationProvider(visitId).future);
      expect(primaryState.visit.id, visitId);
      ...
      final otherState = await container.read(visitDocumentationProvider(otherVisitId).future);
      expect(otherState.visit.id, otherVisitId);
      expect(otherState.subjective, 'Other visit');

      expect(container.read(visitDocumentationProvider(visitId)).value!.visit.id, visitId);
    });
  });
}
