...
final insuranceProvidersProvider =
    AsyncNotifierProvider<InsuranceProvidersNotifier, List<InsuranceProvider>>(
      InsuranceProvidersNotifier.new,
    );

class InsuranceProvidersNotifier
    extends AsyncNotifier<List<InsuranceProvider>> {
  @override
  Future<List<InsuranceProvider>> build() async {
    ...
    return ref
        ...
        .listProviders(onlyActive: false);
  }
  ...
  Future<void> deactivate(String providerId) async {
    await ref
        .read(insuranceProviderRepositoryProvider)
        .deactivateProvider(providerId: providerId);
    await reload();
  }
}

/// Active providers only — for invoice editor insurance selector.
final activeInsuranceProvidersProvider =
    FutureProvider.autoDispose<List<InsuranceProvider>>((ref) async {
      return ref
          .read(insuranceProviderRepositoryProvider)
          .listProviders(onlyActive: true);
    });
