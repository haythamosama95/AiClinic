...
/// Row actions menu for an invoice ledger table row.
class InvoiceRowContextMenu extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final policy = InvoiceActionPolicy(
      status: row.status,
    ...
    final entries = <AppMenuEntry>[
      AppMenuItem(
        ...
        onSelect: () => context.nav.pushBillingInvoiceDetail(row.id),
      ),
  ...
  Widget _buildMenuItemButton(
    ...
    return MenuItemButton(
      ...
      child: Tooltip(
        message: item.disabled ? (item.disabledReason ?? item.label) : item.label,
  ...
  Future<void> _voidInvoice(BuildContext context, WidgetRef ref) async {
    final reason = await VoidInvoiceDialog.showForRow(context, row: row);
    if (reason == null) {
      return;
    }

    try {
      await ref.read(invoiceLedgerProvider(row.id)).voidInvoice(reason: reason);
      ...
      if (toastContext != null && toastContext.mounted) {
        appToast(
          toastContext,
          AppToastInput(
            message: billingMessageForRpc(
              RpcFailure(RpcResult(success: false, errorCode: 'STALE_INVOICE', errorMessage: '')),
            ),
