...
final invoiceBranchFilterOptionsProvider = FutureProvider<List<({String id, String name})>>((ref) async {
  final auth = ref.watch(authSessionProvider);
  final organizationId = auth.context?.organizationId;
  final branchIds = auth.context?.branchIds ?? const <String>[];
  if (organizationId == null || organizationId.isEmpty || branchIds.isEmpty) {
    return const [];
  }
  ...
  return [
    for (final branchId in branchIds)
      (
        id: branchId,
        name: branches.where((branch) => branch.id == branchId).map((branch) => branch.name).firstOrNull ?? branchId,
      ),
  ];
});
...
class InvoiceListControlsBar extends ConsumerWidget {
  const InvoiceListControlsBar({required this.controls, required this.onApply, super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final branches = branchesAsync.value ?? const <({String id, String name})>[];

    String branchName(String branchId) {
      return branches.where((branch) => branch.id == branchId).map((branch) => branch.name).firstOrNull ?? branchId;
    }

    final activeFilters = controls.activeFilters(
      multiBranch: multiBranch,
      branchName: branchName,
      onRemoveStatus: () => onApply(controls.copyWith(clearStatus: true, page: 1)),
      onRemoveBranch: () => onApply(controls.copyWith(clearBranch: true, page: 1)),
    ...
    return AppListControlBar(
      ...
      onSearchChange: (search) => onApply(controls.copyWith(search: search, page: 1)),
      debounceMs: 300,
      ...
      onClearFilters: controls.filterActiveCount > 0
          ? () => onApply(controls.copyWith(clearStatus: true, clearBranch: true, page: 1))
          : null,
          ...
          AppFilterMenuSection(
            ...
            options: [
              ...
              for (final status in InvoiceStatus.values)
                AppFilterMenuOption(value: status.wireValue, label: status.label),
            ],
            onChange: (value) {
              final status = value == 'all' ? null : InvoiceStatus.tryParse(value);
          ...
          if (multiBranch)
            AppFilterMenuSection(
              ...
              options: [
                ...
                for (final branch in branches) AppFilterMenuOption(value: branch.id, label: branch.name),
              ],
              onChange: (value) {
      ...
      onClearAll: controls.isFiltered
          ? () => onApply(InvoiceListControls.defaultControls.copyWith(pageSize: controls.pageSize))
          : null,
    );
  }
}
