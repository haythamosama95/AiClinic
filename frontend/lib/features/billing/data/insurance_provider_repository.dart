...
class InsuranceProviderRepository with AppRpcInvoker {
  ...
  Future<List<InsuranceProvider>> listProviders({
    bool onlyActive = true,
  }) async {
    final result = await invokeRpc('list_insurance_providers', {
  ...
  Future<String> upsertProvider({
    ...
    final providerId =
        result.data?['provider_id']?.toString() ?? result.data?.toString();
    if (providerId == null || providerId.isEmpty) {
      throw StateError(
        'Insurance provider upsert returned an unexpected shape.',
      );
    }
    return providerId;
  }
  ...
  Future<void> deactivateProvider({required String providerId}) async {
    ...
    await invokeRpc('insurance_provider_deactivate', {
      'p_id': providerId.trim(),
    });
  }
...
final insuranceProviderRepositoryProvider =
    Provider<InsuranceProviderRepository>((ref) {
