...
/// Shared clinic setup body used by the bootstrap dialog and legacy setup page.
class ClinicSetupDialogContent extends ConsumerWidget {
  const ClinicSetupDialogContent({this.showPageHeader = true, this.showCompletedBanner = true, super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final body = hydration.when(
      loading: () => _SetupHydrationLoading(colors: colors, showPageHeader: showPageHeader),
      error: (error, _) => _SetupHydrationError(
        colors: colors,
        showPageHeader: showPageHeader,
        onRetry: () => ref.invalidate(clinicSetupHydrationProvider),
      ),
      data: (_) => _SetupBody(
        setupDone: setupDone,
...
class _SetupHydrationLoading extends StatelessWidget {
  const _SetupHydrationLoading({required this.colors, required this.showPageHeader});
...
class _SetupHydrationError extends StatelessWidget {
  const _SetupHydrationError({required this.colors, required this.showPageHeader, required this.onRetry});
...
class _SetupBody extends StatelessWidget {
  const _SetupBody({
    ...
    required this.showCompletedBanner,
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        if (setupDone && showCompletedBanner) ...[
          const SizedBox(height: AppSpacing.space6),
          DecoratedBox(
