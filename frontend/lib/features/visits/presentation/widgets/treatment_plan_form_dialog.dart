...
class _TreatmentPlanFormDialogState extends ConsumerState<TreatmentPlanFormDialog> {
  ...
  Future<List<AppComboboxItem>> _searchMedications(String query) async {
    try {
      final items = await ref.read(medicationSearchProvider(query).future);
  ...
  Widget build(BuildContext context) {
    final frequencyOptions = [
      for (final option in treatmentFrequencyOptions) AppSelectOption(value: option.value, label: option.label),
    ];
    final durationOptions = [
      for (final option in treatmentDurationOptions) AppSelectOption(value: option.value, label: option.label),
    ];
