...
class VisitSubmittedCombinedConfirmation extends StatefulWidget {
  const VisitSubmittedCombinedConfirmation({
...
class _VisitSubmittedCombinedConfirmationState extends State<VisitSubmittedCombinedConfirmation>
  ...
  Widget build(BuildContext context) {
    ...
    final kind = widget.confirmation.kind;
    final actionLabel = kind == VisitConfirmationKind.edited ? 'Updated' : 'Filed';
    final filedAt = DateFormat('EEEE, MMM d, yyyy · h:mm a').format(widget.confirmation.displayTimestamp.toLocal());
    final visitDateLabel = DateFormat('MMM d, yyyy').format(widget.confirmation.visitDate.toLocal());
    final slotLabel = _formatSlot(widget.confirmation.appointmentStart, widget.confirmation.appointmentEnd);
                  ...
                  children: [
                    ...
                    Text.rich(
                      TextSpan(
                        ...
                        children: _bodySpans(kind, widget.confirmation.patientName),
                      ),
                    ...
                    _FilingStub(
                      kind: kind,
                      filingReference: widget.confirmation.filingReference,
                      branchName: widget.confirmation.branchName,
                      doctorName: widget.confirmation.doctorName,
                      visitDateLabel: visitDateLabel,
                      slotLabel: slotLabel,
                    ),
                    if (widget.invoiceSummary != null) ...[
                      const SizedBox(height: AppSpacing.space5),
