...
import 'package:ai_clinic/features/clinic-management/presentation/components/staff_branch_multi_select.dart';
...
/// Staff form fields for clinic-management dialogs (uses [StaffBranchMultiSelect]).
class StaffFormDialogFields extends StatelessWidget {
  const StaffFormDialogFields({
    required this.mode,
  ...
  Widget build(BuildContext context) {
    ...
    final branchOptions = [
      for (final branch in activeBranches)
        AppComboboxItem(
          id: branch.id,
          label: branch.code == null || branch.code!.isEmpty
              ? branch.name
              : '${branch.name} (${branch.code})',
        ),
    ];
    final selectedBranches =
        branchOptions.where((option) => values.branchIds.contains(option.id)).toList();
              ...
              child: AppFormField(
                ...
                child: Directionality(
                  ...
                  child: AppTextInput(
                    ...
                    onChanged: (next) =>
                        onChanged(values.copyWith(phone: next.replaceAll(RegExp(r'\D'), ''))),
