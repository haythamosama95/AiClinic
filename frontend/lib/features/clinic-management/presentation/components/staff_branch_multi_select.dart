...
/// Staff-form [AppMultiSelect] variant that hides the inline query field until
/// the user focuses the control, preventing a blank trailing row in [Wrap].
class StaffBranchMultiSelect extends StatefulWidget {
  const StaffBranchMultiSelect({
    this.id,
  ...
  @override
  State<StaffBranchMultiSelect> createState() => _StaffBranchMultiSelectState();
}

class _StaffBranchMultiSelectState extends State<StaffBranchMultiSelect> {
  ...
  bool get _showQueryField =>
      widget.value.isEmpty || _controller.text.isNotEmpty || _focused;
  ...
  List<AppComboboxItem> get _filtered {
    ...
    return widget.options.where((option) {
      ...
      return option.label.toLowerCase().contains(lower) ||
          (option.meta?.toLowerCase().contains(lower) ?? false);
    }).toList();
  }
  ...
  void _toggle(AppComboboxItem item) {
    ...
    final next =
        exists ? widget.value.where((v) => v.id != item.id).toList() : [...widget.value, item];
                    ...
                    children: [
                      for (final item in widget.value)
                        AppChip(
                          ...
                          disabled: widget.disabled,
                          onRemove: widget.disabled ? null : () => _remove(item.id),
