...
- [ ] T001 Verify local Supabase stack runs and latest migration applied (`backend/local`, `supabase status`); confirm `backend/supabase/migrations/20260717120000_billing_currency.sql` is the current highest timestamp before adding new migrations
- [ ] T002 [P] Confirm Flutter_repo `flutter pub get` succeeds and existing patient/invoice/appointment feature packages compile (`cd frontend && flutter analyze` clean) before changes
