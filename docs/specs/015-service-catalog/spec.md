...
This feature **modifies the invoice item contract** from `docs/specs/007-billing`: the free-text `description` entry path is removed entirely and replaced by a required catalog selection, and each invoice item gains a `service_id` link plus price-snapshot fields. Previously issued invoices remain untouched.
...
- **NFR-005**: All monetary values MUST be stored as exact decimals with fixed scale 2 (no floating point) across `default_price`, `price_override`, `promotion_price`, and invoice-item snapshots, consistent with `docs/specs/007-billing` NFR-007.
...
- **Invoice Item** (modified from `docs/specs/007-billing`): Itemized service line on an invoice; now carries a required `service_id` link and immutable snapshot fields (`service_name`, `unit_price`, `quantity`, `total_price`).
...
- `docs/specs/002-auth-rbac`, `docs/specs/003-org-branch-management`, `docs/specs/004-patient-management`, `docs/specs/006-visit-medical-records`, and `docs/specs/007-billing` are implemented; this feature builds on the existing organization, branch, permission, audit, and billing foundations.
...
- Monetary amounts use exact decimals at scale 2 (per `docs/specs/007-billing` NFR-007); each organization operates in a single configured currency (multi-currency is out of scope).
