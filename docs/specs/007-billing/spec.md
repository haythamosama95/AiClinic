...
- Q: Where does the "Allow partial payments" toggle live in the UI? → A: Inside a new **"Billing"** subsection of the existing **organization-level Settings page** introduced in V1-2 (`docs/specs/003-org-branch-management`). The setting is organization-scoped (applies to all branches uniformly); no branch-level override exists in V1-6. The "Billing" subsection is the canonical home for future organization-wide billing toggles.
...
- **FR-026**: The system MUST derive requirements from the architecture documents listed under Required Architecture Docs and treat `docs/specs/operations/billing.spec.md` as an external reference until authored.
...
- `docs/specs/operations/billing.spec.md` is referenced by the roadmap but is not yet present. This specification captures invoice and payment expectations for V1-6 until that shared operations spec is authored.
- `docs/specs/006-visit-medical-records` is a hard prerequisite: visit `completed` status drives visit-linked invoice creation.
- `docs/specs/004-patient-management` is a hard prerequisite: patient registry must exist for invoice patient linkage.
- `docs/specs/003-org-branch-management` and `docs/specs/002-auth-rbac` are hard prerequisites: active branch, staff identities, permissions, and session management must exist.
...
- MUST build on completed `docs/specs/002-auth-rbac`, `docs/specs/003-org-branch-management`, `docs/specs/004-patient-management`, `docs/specs/005-appointment-management`, and `docs/specs/006-visit-medical-records`.
...
- `docs/specs/002-auth-rbac`, `docs/specs/003-org-branch-management`, `docs/specs/004-patient-management`, `docs/specs/005-appointment-management`, and `docs/specs/006-visit-medical-records` are implemented.
...
- `docs/specs/operations/billing.spec.md` will be authored later; this feature spec is authoritative for V1-6 until that shared spec exists.
