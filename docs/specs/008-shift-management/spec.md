...
**Feature Branch**: `008-shift-management`
...
- **FR-023**: The system MUST derive requirements from the architecture documents listed under Required Architecture Docs and treat `docs/specs/operations/shifts.spec.md` as an external reference until that shared operations spec is authored.
...
- `docs/specs/operations/shifts.spec.md` is referenced by the roadmap but is not yet present. This specification captures shift management expectations for V1-7 until that shared operations spec is authored.
- `docs/specs/007-billing` is a sequential prerequisite in the V1 roadmap; billing behavior is unchanged by V1-7.
- `docs/specs/003-org-branch-management` is a hard prerequisite: branches, staff members, staff–branch assignments, and active branch context must exist.
- `docs/specs/002-auth-rbac` is a hard prerequisite: session management, `shifts.manage` permission seed, and branch-scoped JWT claims must exist.
