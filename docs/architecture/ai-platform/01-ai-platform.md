...
5. **Invoice generation moves inside the platform** — reversing the billing row of
   ...
   writes an immutable `invoice` record per installation. The version active for a period is the
   latest `credit_price` row whose `active_from` is at or before the period start; a version
   activated with an `active_from` inside or after the period does not apply to that month, and a
   later activation never reprices a period already closed. If no row has `active_from` at or
   before the period start, close cannot price the period and issues no invoice for it — the same
   outcome shape as a zero-consumption period — rather than inventing a default price or currency.
   **Payment collection remains outside**:
