...
| **Revocation** | Clinic **owner or administrator** | `public.revoke_installation_key(kid)` | Sets `revoked_at` on that key row. Rejects with `CANNOT_REVOKE_LAST_ACTIVE_KEY` when the target is the last active key — rotate a replacement first. Idempotent when the key is already revoked. The platform rejects any AAT whose header `kid` maps to a revoked key, regardless of `exp`. |
