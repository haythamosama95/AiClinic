...
5. [Behavioral verification](#5-behavioral-verification)
   ...
   - [5.3 Ordered probes](#53-ordered-probes)
     - [5.3.1 Reset to a known Quota DO](#531-reset-to-a-known-quota-do)
     - [5.3.2 Entitle with known ceilings](#532-entitle-with-known-ceilings)
     - [5.3.3 First admit and credit](#533-first-admit-and-credit)
     - [5.3.4 JTI replay](#534-jti-replay)
     - [5.3.5 Idempotent replay after credit](#535-idempotent-replay-after-credit)
     - [5.3.6 Soft threshold and request-quota exhaustion](#536-soft-threshold-and-request-quota-exhaustion)
     - [5.3.7 Token and cost ceilings](#537-token-and-cost-ceilings)
     - [5.3.8 Period reset](#538-period-reset)
     - [5.3.9 Cancel and zero-usage credit](#539-cancel-and-zero-usage-credit)
     - [5.3.10 Abandoned admission sweep](#5310-abandoned-admission-sweep)
     - [5.3.11 Release on journal-insert failure](#5311-release-on-journal-insert-failure)
     - [5.3.12 Grace admission and the D1 cap](#5312-grace-admission-and-the-d1-cap)
     - [5.3.13 Concurrency limit](#5313-concurrency-limit)
     - [5.3.14 Ephemeral sweep and idempotency TTL](#5314-ephemeral-sweep-and-idempotency-ttl)
...
Live probes against a running local Worker Quota DO (`GatewayObject`, `idFromName(installationId)`). There is no operator dump of the DO `"state"` key — [runbook §6.4](../04-ai-platform-operator-runbook.md#64-durable-objects-quota--admission) has no Wrangler recipe, and this document does not add a debug RPC. Every field is read through **observable effects**: D1 `ai_request` / `usage_event` / `grace_admission_queue`, HTTP/SSE outcomes (`quota_exhausted`, JTI `unauthenticated`, idempotent replay), and Worker logs on `npm run dev`. Each probe is an operator action and the outcome you should see — not a unit test. Run **[§5.3](#53-ordered-probes) top to bottom** on a throwaway installation. If every probe matches, this DO is working.
...
| Claim | Probe |
| ----- | ----- |
| No operator dump of DO `"state"`; observe via D1 / HTTP / SSE / logs | [§5.1](#51-setup), [§5.3.1](#531-reset-to-a-known-quota-do) |
| `periodCounters.requestsUsed` +1 per credit; exhausts when ≥ `request_quota` | [§5.3.3](#533-first-admit-and-credit), [§5.3.6](#536-soft-threshold-and-request-quota-exhaustion) |
| `periodCounters.tokensUsed` += credited tokens | [§5.3.3](#533-first-admit-and-credit), [§5.3.7](#537-token-and-cost-ceilings) |
| `periodCounters.costUsed` += credited cost | [§5.3.3](#533-first-admit-and-credit), [§5.3.7](#537-token-and-cost-ceilings) |
| `periodCounters.inFlight` +1 at admit, −1 at credit/release/abandoned sweep | [§5.3.3](#533-first-admit-and-credit), [§5.3.9](#539-cancel-and-zero-usage-credit), [§5.3.10](#5310-abandoned-admission-sweep), [§5.3.11](#5311-release-on-journal-insert-failure) |
| `periodBounds.period_start` from entitlement snapshot | [§5.3.3](#533-first-admit-and-credit) (`usage_event.period`) |
| `periodBounds.period_end` from entitlement snapshot | [§5.3.6](#536-soft-threshold-and-request-quota-exhaustion) (`period_reset`) |
| `maybeResetPeriod` on admit and credit when bounds change | [§5.3.8](#538-period-reset) |
| `jtiReplay[jti].expiresAt` — same AAT cannot admit twice | [§5.3.4](#534-jti-replay) |
| JTI window 2h already ≥ 10-minute max AAT lifetime | [§5.3.4](#534-jti-replay), [§5.3.14](#5314-ephemeral-sweep-and-idempotency-ttl) |
| `idempotency[key]` `{ expiresAt, requestReference, state, requestId }` | [§5.3.3](#533-first-admit-and-credit), [§5.3.5](#535-idempotent-replay-after-credit) |
| Idempotency states are `admitted` \| `completed` \| `failed` \| `cancelled` only | [§5.3.5](#535-idempotent-replay-after-credit), [§5.3.9](#539-cancel-and-zero-usage-credit), [§5.3.10](#5310-abandoned-admission-sweep) |
| `admitted` at admit; credit may set `completed` / `failed` / `cancelled` | [§5.3.3](#533-first-admit-and-credit), [§5.3.9](#539-cancel-and-zero-usage-credit), [§5.3.10](#5310-abandoned-admission-sweep) |
| Abandoned sweep may set `failed` (not a completed placeholder) | [§5.3.10](#5310-abandoned-admission-sweep) |
| `creditedRequests[requestId].{ expiresAt }` — second credit does not double-count | [§5.3.5](#535-idempotent-replay-after-credit), [§5.3.14](#5314-ephemeral-sweep-and-idempotency-ttl) |
| `admittedRequests[requestId]` `{ requestReference, admittedAt, entitlement }` | [§5.3.3](#533-first-admit-and-credit), [§5.3.8](#538-period-reset), [§5.3.10](#5310-abandoned-admission-sweep) |
| `boundInstallationId` set on first admit | [§5.3.3](#533-first-admit-and-credit) |
| `EPHEMERAL_HORIZON_MS` = 2h | [§5.3.14](#5314-ephemeral-sweep-and-idempotency-ttl) |
| `CONCURRENCY_LIMIT` = 16 | [§5.3.13](#5313-concurrency-limit) |
| `GRACE_ADMISSION_CAP` = 5, D1 not this DO | [§5.3.12](#5312-grace-admission-and-the-d1-cap) |
| RPC `admission` — `admitted` | [§5.3.3](#533-first-admit-and-credit) |
| RPC `admission` — `replay` (JTI) | [§5.3.4](#534-jti-replay) |
| RPC `admission` — `idempotent` | [§5.3.5](#535-idempotent-replay-after-credit) |
| RPC `admission` — `quota_exhausted` | [§5.3.6](#536-soft-threshold-and-request-quota-exhaustion), [§5.3.7](#537-token-and-cost-ceilings) |
| RPC `admission` — `concurrency_exhausted` | [§5.3.13](#5313-concurrency-limit) |
| RPC `credit` — settle usage; `inFlight` released; zero usage valid | [§5.3.3](#533-first-admit-and-credit), [§5.3.9](#539-cancel-and-zero-usage-credit) |
| Credit `idempotencyState` `completed` / `failed` / `cancelled` (`partial` → `cancelled`) | [§5.3.3](#533-first-admit-and-credit), [§5.3.9](#539-cancel-and-zero-usage-credit), [§5.3.10](#5310-abandoned-admission-sweep) |
| Credit re-runs `maybeResetPeriod`; slides `expiresAt` | [§5.3.8](#538-period-reset), [§5.3.14](#5314-ephemeral-sweep-and-idempotency-ttl) |
| RPC `release` — compensate when stage-9 journal INSERT fails | [§5.3.11](#5311-release-on-journal-insert-failure) |
| Admission sets `idempotency.expiresAt` and `admittedAt` to the same now+2h | [§5.3.14](#5314-ephemeral-sweep-and-idempotency-ttl) |
| `sweepEphemeral`: abandoned-admission first, then delete expired maps | [§5.3.10](#5310-abandoned-admission-sweep), [§5.3.14](#5314-ephemeral-sweep-and-idempotency-ttl) |
| Abandoned: drop admitted row, `inFlight` −1, mark matching idempotency `failed`, slide `expiresAt` | [§5.3.10](#5310-abandoned-admission-sweep) |
| Retry after original horizon but inside slid window stays idempotent; after slid window the key admits fresh | [§5.3.14](#5314-ephemeral-sweep-and-idempotency-ttl) |


### 5.3 Ordered probes

#### 5.3.1 Reset to a known Quota DO
...
**Expect:** both counts `0`. `entitlement.status = pending`. You cannot `SELECT` the DO `"state"` blob. If this `installation_id` was used on this machine before, mint a **new** Stage 2 id and re-enroll — leftover `requestsUsed` will fail later ceilings.

#### 5.3.2 Entitle with known ceilings
...
#### 5.3.3 First admit and credit

**Do:** mint AAT **T0**. POST with `x-idempotency-key` **KEY0** (the template in [§5.1](#51-setup)). Watch Worker logs.

**Expect:** SSE `accepted` then a terminal (`completed` on a wired fake/success path). Logs include `Admission granted` then `Credit applied` (`partial` false on success). D1: exactly one `ai_request` for **KEY0**, `request_id` **REQ0** (DO-issued UUID), `request_reference` **R0**, `state` terminal, `routing_tier = standard` (usage was still 0 at admit, so soft threshold did not fire). One `usage_event` for **REQ0**: `tokens` and `cost` ≥ 0, `period = 2026-08` (from `period_start`, not wall-clock month-of-credit). That row is the operator-visible `requestsUsed` / `tokensUsed` / `costUsed` increment and the `periodBounds.period_start` snapshot. `inFlight` is 0 again — the next **new** key in [§5.3.6](#536-soft-threshold-and-request-quota-exhaustion) can admit. `boundInstallationId` is now **I0** (same installation keeps working; a mismatch is not reachable because each installation has its own DO).
...
**Expect:** admitted and credited. New `usage_event.period = 2026-09`. `requestsUsed` is 1 in the **new** period (`request_quota = 100` leaves headroom for [§5.3.9](#539-cancel-and-zero-usage-credit)–[§5.3.13](#5313-concurrency-limit)). Credit re-ran `maybeResetPeriod` from the snapshot (Worker sends admission-time entitlement on credit; after cache refresh that snapshot is the new bounds). You cannot inspect `admittedRequests[].entitlement` on the wire; the new `period` column is the effect. Mid-flight mutation of D1 between admit and credit is not operator-holdable on the fake adapter.

#### 5.3.9 Cancel and zero-usage credit

**Do:** mint **T6**. Start POST **KEY6** with `curl -N` and abort the client as soon as SSE `accepted` appears (Ctrl+C / close the socket).

**Expect:** if you closed before the fake adapter finished: D1 `state = Cancelled`, `usage_event` for that `request_id` with tokens/cost allowed to be `0`, Worker `Credit applied` with `partial: true`, idempotency `cancelled`. A remint + same **KEY6** replays SSE `cancelled`, not a second charge. Zero usage still released `inFlight` (another **new** key can admit while `request_quota` remains). If the fake path already completed, D1 is `Completed` instead — the cancel window is milliseconds; do not invent a hang endpoint. Retry this after [§5.3.8](#538-period-reset) while a request slot remains.

#### 5.3.10 Abandoned admission sweep
...
#### 5.3.11 Release on journal-insert failure
...
#### 5.3.12 Grace admission and the D1 cap
...
#### 5.3.14 Ephemeral sweep and idempotency TTL

JTI, idempotency, `creditedRequests`, and abandoned `expiresAt` all use `EPHEMERAL_HORIZON_MS = 7_200_000` (2h). Identity already rejects `exp − iat > 600` (`MAX_AAT_LIFETIME_SECONDS`), so a still-valid AAT cannot be presented after its JTI entry would have expired — [§5.3.4](#534-jti-replay) is the live half of that bound; the counterfactual (valid token, expired JTI) cannot happen.

**Do:** more than 2 hours after the **credit** of **KEY0** (credit slides `expiresAt` to now+2h; on the fake path that is essentially 2h after [§5.3.3](#533-first-admit-and-credit)). Mint a new AAT. POST **KEY0** again.
...
On this adapter, credit follows admit immediately, so you cannot show “retry after the original admission horizon but still inside the slid credit window” as a distinct interval — holding a reservation for ~2h then crediting it is the missing window. The slid **failed** window in [§5.3.10](#5310-abandoned-admission-sweep) is the operator-visible slide: after the sweep mark, a retry inside the next 2h still replays `failed`; after that 2h the key admits fresh.
