...
9. [Worked Examples](#9-worked-examples)
  ...
  - [Example D — An open chat question](#94-example-d--an-open-chat-question)
...
| **The surface names the capability** | A button press names a capability by being that button; an open chat window names one assistant capability whose intent is the typed message. The client never infers a capability from what a user wrote — that would put intent inference in the layer that knows nothing about AI.             |
| **Two interaction modes**            | `single_shot` covers every button-invoked feature: context is known from the manifest before submitting. `conversational` covers the chat assistant: many turns, and the platform asks the client for context mid-conversation. Modes are a manifest flag, so neither shape leaks into the other. |
...
*When* the platform declares its keys differs by interaction mode, but *who* declares them never does.
A button-invoked capability lists them in its manifest ahead of time. A chat capability lists a
permitted set it may draw on, and names the specific keys during the conversation, once a model has read
...
| **Conversation store**  | Chat surfaces only: holds an open conversation's transcript locally, resupplies it on each turn, discards it when the chat closes. It never interprets the transcript.              |


None of these contains prompt text, model names, provider names, or AI business rules. That is the
acceptance test for the client layer, and a CI lint enforces it. The conversation store is included in
that test: holding a transcript is allowed, reading meaning from it is not.
...
| **Entitlement, quota, and rate control** | Whether the request is allowed to cost money, and whether it is a replay — see [§8.2](#82-rate-limiting-quota-and-cost-are-different-things).                                    |
...
| **Telemetry emitter**                    | Traces and structured logs. Never prompts, context, or credentials. Metrics are queried from the journal rather than emitted separately.                                         |
...
installation, it holds counters plus the short-lived replay and idempotency sets, and it hibernates
when idle. There is no per-request state anywhere.
...
There are three stores and one credential binding. Each is present because it has a property none of
the others has.


| Store               | Holds                                                                                                                             | Never holds                            | The property nothing else has                 |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- | --------------------------------------------- |
| **D1**              | Installations, keys, entitlements, routing policy, request journal metadata, usage ledger, rollups, counters, control-plane audit | Clinic business records, large text    | Relational, queryable truth                   |
| **R2**              | One payload envelope per request: composed prompt, context, raw provider exchanges, validated result                              | Anything needed on the hot path        | Unbounded object size, cheap storage          |
| **Durable Object**  | Per-installation quota and concurrency counters, plus the `jti` replay set and idempotency records                                | Long-term records, per-request objects | Serialized, strongly consistent counting      |
| **Secrets binding** | Provider API keys and signing material                                                                                            | Anything logged or journaled           | Credential isolation (a binding, not a store) |


Alongside them sits the **config cache** — an in-isolate memory map with a short TTL, filled from D1
on a miss — holding installations, keys, entitlements, grants, kill switches, and the active routing
policy. It is a latency optimization and owns nothing.

Three rules govern the write path:

- **Payloads go to R2 and pointers go to D1.** One D1 row per request on the request path;
per-attempt detail and the payload envelope are written after the client has its answer.
- **One request produces one R2 object.** R2 meters write operations far more expensively than reads,
and all four payload sections are assembled after the response anyway, so splitting them into
separate objects would multiply the platform's scarcest metered resource for no benefit.
- **Metrics are derived, not written.** Every dashboard dimension already exists on a journal row, so
there is no second metrics store to keep in sync — and the dashboard can never disagree with the
audit trail.

The property that explains the whole set: **D1 is the only authoritative store.** R2 holds bytes that
D1 points to, the Durable Object holds counters that settle into D1's ledger, and the config cache
holds copies of D1 rows. That test is also what removed two stores from earlier drafts — Workers KV
and Analytics Engine — since neither held anything D1 did not already describe.
...
Two things are decided *before* any of this, and they are what keep the client dumb. The **capability
is named by the surface the user touched**, never inferred from what they typed. And the **context keys
come from the platform** — either from the manifest ahead of time, or from the platform asking for them
mid-conversation. The client resolves keys; it never decides which ones matter.
...
| **Submit request**       | Create an AI request for a capability, with the user intent, the resolved context payload, an idempotency key, and a capability version pin. For a chat capability the same surface also carries the conversation id and the transcript so far — there is no separate chat endpoint. |
...
4. **Exactly one terminal event ends every stream**: `completed` with the validated result, `failed`
  with a taxonomy code, `cancelled`, or — for chat capabilities only — `context_requested` naming the
   clinic data the assistant needs. Completion is never inferred from silence.
...
| Input         | `request_too_large`, `context_required`, `context_invalid`, `conversation_budget_exhausted` | Narrow the input, resolve the missing keys and resubmit once, or start a fresh conversation. |
...
`context_requested` is deliberately **not** in this table. The turn ran, the provider was called, and
the tokens were spent — the platform is asking for data, not reporting a fault. It is a terminal event
kind, not an error code, so a normal chat turn never looks like a failure in the journal.
...
sequenceDiagram
    ...
    GW->>GW: guard: shape, identity, entitlement, rate limit,<br/>capability, context, cost, then one admission call
    ...
    GW->>GW: record terminal state, credit usage,<br/>then write detail and the payload envelope
...
| ---------- | --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| **Guard**  | Shape → identity → entitlement → rate limit → capability resolve → context validate → cost pre-flight → **admission** | This request is well-formed, authentic, permitted, affordable, and not a replay | Tens of milliseconds; one round trip and one insert           |
...
| **After**  | Attempt detail and the usage ledger row to D1, one payload envelope to R2                                             | Diagnostic depth                                                                | Runs after the client has its answer; never fails the request |


**Admission** is one stage because it is one round trip. Four questions — is the token id fresh, is
this idempotency key new, is there budget left, is there concurrency headroom — are all scoped to a
single installation and all need serialized truth, so the per-installation Quota Durable Object
answers them together. Everything before it is CPU or memory-cache work, which is what keeps rejecting
a bad request nearly free.
...
or failed. Rejections from the guard are tallied in platform counters rather than journaled as
requests.
...
stateDiagram-v2
    ...
    Validating --> AwaitingContext: context request<br/>(chat only)
...
transition is journaled with a timestamp.

`AwaitingContext` is terminal in the full sense: *that request is over*. A chat conversation continues
as a new request, grouped with the previous one by a conversation id. Calling it terminal rather than a
pause is what keeps the platform holding no in-flight conversation state.

### 6.4 Three flows worth knowing
...
lets platform releases and desktop releases move on different schedules. It applies to button-invoked
capabilities only, and a second `context_required` is a real defect.
- **Chat context negotiation.** Superficially similar, genuinely different. The platform cannot know
what a typed question needs until a model has read it, so a chat turn may end with
`context_requested` naming keys and their arguments. The client resolves them under the user's own
RLS, appends them to the transcript, and submits the next turn. The rounds are capped by the
manifest. Compare the two: `context_required` fires *before* any inference because the client was
stale; `context_requested` fires *after* one because only the model could know.
...
Seven ideas cut across every component. Missing any of them makes the design look arbitrary.
...
Distinct concerns with distinct consistency needs. The dividing line is **approximate versus
serialized**, not conceptual tidiness: everything that can tolerate being wrong by a little stays
local and free, and everything that cannot shares one round trip.


| Concern             | Question                                   | Consistency           | Mechanism                                                                                                                                    |
| ------------------- | ------------------------------------------ | --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| **Rate limiting**   | Too many requests too fast?                | Approximate by design | Rate-limiting binding on composite keys (installation, installation+actor, installation+capability). No I/O, so abuse stays cheap to refuse. |
| **Cost ceiling**    | Is this single request too expensive?      | Local estimate        | A pre-flight token estimate against the capability's token budget, before any egress. Tokens, never currency — the Worker holds no price table. |
| **Quota / budget**  | Has this clinic consumed what it paid for? | Serialized            | Per-installation Quota Durable Object — counters credited with *actual* usage after each request, backed by an append-only ledger.           |
| **Concurrency cap** | Too many simultaneous inferences?          | Serialized            | A counter in the same Durable Object.                                                                                                        |
| **Replay**          | Has this token id been used before?        | Serialized            | A short-lived set in the same Durable Object.                                                                                                |
| **Idempotency**     | Has this request already been made?        | Serialized            | A short-lived record in the same Durable Object; a repeat returns the original request.                                                      |


The last four are asked together in the guard's single **admission** call, because they are all
installation-scoped and all need the same serialization. Keeping them in separate stores would mean
separate round trips for facts one object could settle at once.
...
validation results, terminal state, plus a pointer to the R2 envelope holding the exact prompt,
context, and result.
...
Logs are for aggregate health and may be sampled; the journal is for individual truth and may not.
Metrics sit on the journal's side of that line — they are queried from it rather than written to a
store of their own, so a dashboard can never disagree with the audit trail.
...
### 8.7 The chat surface is a capability, and the client holds the transcript

The product needs two AI shapes: buttons tied to a screen ("Summarize this visit") and an open chat
window. They are the same pipeline with one manifest flag between them.


|                      | `single_shot` — buttons                            | `conversational` — chat                   |
| -------------------- | -------------------------------------------------- | ----------------------------------------- |
| Capability chosen by | The button pressed                                 | The chat window; one assistant capability |
| User intent is       | The selection or the record's identifiers          | The literal typed message                 |
| Context known        | From the manifest, before submitting               | Negotiated during the conversation        |
| Turns                | One                                                | Many, each a separate request             |
| Context negotiation  | No — `context_required` stays a stale-cache repair | Yes, capped by the manifest               |


Four properties make this an extension rather than a second architecture:

1. **The client holds the transcript and resupplies it every turn.** So the platform holds no
  conversation state, adds no store, and keeps a per-installation Durable Object as its only stateful
   component. A conversation is a series of independent requests grouped by a conversation id.
2. **The platform asks; it never fetches.** A context request travels to the client, which resolves it
  against Supabase under the requesting user's own session and RLS. The assistant's reach is the
   intersection of the manifest's permitted key set and what that user could already open in the UI, so
   chat can never become a permission bypass.
3. **The loop is bounded by things computed from the request itself** — max history turns and max
  context rounds counted from the supplied transcript, plus the ordinary per-request cost pre-flight,
   which already prices the transcript because the transcript is input.
4. **Everything else is unchanged.** No new pipeline stage, no change to cancellation, idempotency,
  quota, journaling, or the human-acceptance rule. The transcript is untrusted like any context
   payload, and the bound that contains a tampered one is the clinic's own quota.
...
Four end-to-end walkthroughs, each naming every hop and every component involved. They build on each
other: **Example A** is the full path with the fewest moving parts, **Example B** adds clinic data
collection, **Example C** adds streaming, and **Example D** adds an open chat question where the needed
data is not knowable in advance. Where a later example repeats an earlier step it is still listed, but
...
| 9   | **Identity + tenant resolution** | Selects the verification key by the token's issuer claim through the **token verifier port**, then checks the signature, audience, expiry, and clock skew. Loads the installation record and its status.                                                                                                                      | Config cache — an in-isolate map with a short TTL, filled from D1 only on a cold isolate |
| 10  | **Identity** (cont.)             | Builds the immutable **request principal**: installation, organization, branch, actor, role, capability scopes. No later stage may modify it.                                                                                                                                                                                 | —                                                                                        |
| 11  | **Entitlement**                  | Confirms the installation is active and AI-enabled, and that its plan and grants permit `note.polish`.                                                                                                                                                                                                                        | Config cache                                                                             |
| 12  | **Rate control**                 | Evaluates burst limits on the composite keys — installation, installation+actor, installation+capability.                                                                                                                                                                                                                     | Rate-limiting binding, no I/O                                                            |
| 13  | **Capability resolver**          | Resolves `note.polish@v1` to its immutable manifest, honouring the client's version pin; checks lifecycle state and the global, per-installation, per-capability, and per-provider kill switches.                                                                                                                             | Bundled manifest artifacts; config cache for flags                                       |
| 14  | **Context validator**            | The manifest declares no required keys, so there is nothing to validate. If the client had sent extra keys anyway, they would be **dropped here** rather than forwarded.                                                                                                                                                      | —                                                                                        |
| 15  | **Cost pre-flight**              | Estimates input tokens from the intent by byte length (§13.6.2), adds the capability's max output tokens, and compares the total in tokens against the capability's token-denominated per-request cost ceiling.                                                                                                                                                                    | —                                                                                        |
| 16  | **GW** → **DO** (admission)      | The guard's only round trip, carrying four installation-scoped questions at once: is this `jti` unseen, is this idempotency key new, is there budget left in the current period, and is there concurrency headroom?                                                                                                           | Quota Durable Object                                                                     |
| 17  | **DO** → **GW**                  | Admitted. Budget answers yes, yes-but-degraded (soft threshold crossed), or no; the `jti` is recorded, the idempotency key is claimed, and the in-flight counter is incremented — all serialized inside the object. A repeated idempotency key would return the original request here instead of starting a second inference. | Quota Durable Object                                                                     |
...
| 32  | *After the response* — **Journal writer** → **D1** | Writes the per-attempt row (provider, model, latency, tokens, cost, provider request id) and the append-only usage ledger row that backs the DO counters. Outcome, latency, token, and cost metrics need no separate write — they are columns on these rows, and dashboards query them directly.  |
| 33  | *After the response* — **Journal writer** → **R2** | Writes **one** payload envelope for the request: composed prompt, context (empty here), raw provider exchange, and validated result in a single object. One object rather than four, because R2 meters writes far more expensively than reads and all four sections are ready at the same moment. |
...
| 18  | **Cost pre-flight** → **DO**              | The token estimate now includes the rendered context, which is usually the bulk of the input. Then the single admission round trip, as in Example A steps 16–17.                                                                                                                                                                                 |
...
| 25  | **Settle and after**                      | Terminal state to D1, actual usage credited to the DO, then the attempt rows, the usage ledger row, and one payload envelope holding the exact context that was used — as in Example A steps 30–33.                                                                                                                                              |
...
| 2   | **GW** guard                   | Stages 1–8 exactly as in Example A: shape, identity, entitlement, rate control, capability resolve, context validation, cost pre-flight, then the single admission round trip.                        |
...
| 13  | Nothing is written per chunk                | No journal row, no R2 write, no counter update happens per chunk anywhere in the platform. Only aggregates are recorded, after the fact.                                                                                                                     |
...
| 18  | **Settle and after**        | Terminal state to D1, usage credited to the DO, then the attempt rows, the ledger row, and one payload envelope. Time-to-first-token is recorded on the attempt row, so it is queryable per provider and model without a separate metrics write. |
...
### 9.4 Example D — An open chat question

**Scenario.** A clinician opens the assistant and types "what did we prescribe Ahmed at his last
visit?" — attached to no screen and no record. Nothing about this request is knowable before a model
reads the sentence.

**Capability.** `clinic.assistant@v1` — interaction mode `conversational`; permitted context keys
`patient.demographics@v1`, `visit.history@v1`, `medication.active_list@v1`; output mode `prose`; max
context rounds per turn: 2; acceptance mode `advisory_display`.

#### 9.4.1 Leg 1 — the assistant asks for data


| #   | Hop                          | What happens                                                                                                                                                                                                                                                  |
| --- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Clinician → **Surface**      | Types the message and presses send. The surface performs **no interpretation** — it does not parse the name, guess the patient, or choose a capability. The chat window *is* `clinic.assistant@v1`.                                                           |
| 2   | **Surface** → **SDK**        | Supplies the message as the user intent, the client-generated `conversation_id`, and `turn_ordinal` 1. The transcript is empty.                                                                                                                               |
| 3   | **SDK** → **SB** → **GW**    | Token minted or reused, then submit with an **empty context payload** — the manifest declares a permitted set, not a required list, so there is nothing to resolve yet.                                                                                       |
| 4   | **GW** guard                 | The same stages as every other example. The cost pre-flight prices the message and the (empty) transcript. Admission, then the journal row — this leg is a first-class request.                                                                               |
| 5   | **Prompt composer**          | Assembles the assistant's system instruction, its business-rule fragments, the empty transcript, and the typed message rendered as delimited data. It offers the model two valid output shapes: prose, or a **context request** drawn from the permitted set. |
| 6   | **Provider** → **Validator** | The model returns a context request: `patient.demographics@v1` and `medication.active_list@v1`, with the argument "Ahmed". The validator checks it against the shared, platform-owned context-request schema and **drops any key outside the permitted set**. |
| 7   | **Stream broker** → **SDK**  | One terminal `context_requested` event carrying the keys and their arguments. The journal row reaches `awaiting_context`, and the tokens actually spent are credited to the Quota DO — the inference happened, so it is billed.                               |




#### 9.4.2 Leg 2 — the client resolves and the assistant answers


| #   | Hop                                     | What happens                                                                                                                                                                                                                                                                          |
| --- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 8   | **SDK** → **Resolver** → **SB**         | Resolves the two named keys through the same generic registry every other capability uses, **under the clinician's own session and RLS**. If this user may not see Ahmed, the reads return nothing and the assistant simply cannot discuss him.                                       |
| 9   | **SDK** internally                      | Appends the assistant's context request and the resolved payload to the local transcript. Generates a **new** idempotency key — this is a new request, not a retry.                                                                                                                   |
| 10  | **SDK** → **GW**                        | Submit: same `conversation_id`, `turn_ordinal` 2, the transcript, the resolved context.                                                                                                                                                                                               |
| 11  | **GW** guard                            | Additionally counts the consecutive context-request turns at the tail of the supplied transcript against the manifest's limit of 2, and rejects with `conversation_budget_exhausted` if exceeded. The cost pre-flight now prices the transcript too, because the transcript is input. |
| 12  | **Prompt composer → provider → broker** | The context is rendered as delimited typed data, exactly as in Example B, and the answer streams as `text_delta` events like any other prose capability.                                                                                                                              |
| 13  | **Stream broker** → **SDK** → Clinician | One terminal `completed` event with the validated prose. The client appends it to the transcript and renders it. Because the acceptance mode is `advisory_display`, there is no commit control — moving anything into a record would go through the ordinary acceptance RPC.          |
| 14  | **Settle and after**                    | Two journal rows now share one `conversation_id`, each with its own attempt rows, usage ledger row, and payload envelope. Support can read the whole conversation with one indexed query.                                                                                             |


**What this example demonstrates:** the client forwarded a sentence it did not understand, resolved keys
