...
> ⚠️ **Set-specific runtime regressions.** The Display port must not regress on the pitfalls below (consult `docs/ui/memory/ui-runtime-errors.md` only when explicitly instructed). Apply these guards to every widget that touches a Material primitive, an overlay, a `Focus`/`FocusNode`, or `intl` date formatting. Specifically relevant to this plan:
