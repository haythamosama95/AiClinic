...
78. **Revoke success payload timestamp** — the fresh-revoke path returns a NEW `clock_timestamp()` evaluated after the UPDATE (`20260902130100…sql:L55`), which can differ by microseconds from the stored `revoked_at`; only the idempotent path returns the stored value. [Stage 2 #4]
