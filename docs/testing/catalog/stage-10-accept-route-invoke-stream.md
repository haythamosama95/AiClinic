...
|-------|---------|
| ID | S10-005 |
| Journey setup | Setup FRESH. Policy document: catch-all, single target `{provider_id: "bogus-primary", model_id: "bogus-v1", max_attempts: 2, timeout_ms: 30000}` with visit-summary-valid features. Production `resolveProviderPort` maps the unknown id to a **fresh** `new FakeAdapter(["retryable:provider_unavailable"])` **per attempt** — adapters are not shared across attempts. |
| Action | `POST /v1/requests`, idempotency key `s10-005-idem`, trace `s10-005-trace`. Measure wall-clock elapsed. |
| Expected outcome | Events: `accepted` → `failed` `provider_unavailable`, `retry_safe: true`. `setRetryabilityFromClassification` recomputes from the taxonomy (`provider_unavailable` retryable "Yes"), so `processInvokeResult` records **retryable_failure** and the loop retries: both attempts `error_code "provider_unavailable"` (each construction scripts the same token; the empty-queue `internal_error` branch is a fixture artifact of a shared queue and does not fire). Elapsed ≥ ~100 ms (jittered backoff `100×2^0` + up to 50% jitter, via `wallClockSleeper`). Chain exhausted → `provider_unavailable`. |
| Side effects | D1 `ai_attempt`: 2 rows — `(attempt_no 1, provider "bogus-primary", outcome "retryable_failure", error_code "provider_unavailable")`, `(attempt_no 2, outcome "retryable_failure", error_code "provider_unavailable")`. `ai_request` → `Failed/provider_unavailable`. Credit once, `partial: true`, idempotency `failed`. One envelope; attempt payloads `{fake: true, outcome: "retryable:provider_unavailable"}` on both. |
| Code reference | ai-platform/src/worker.ts:L343-L366 — `resolveProviderPort` unknown-id fallback, per-attempt construction; ai-platform/src/provider/classify.ts:L10-L13 — `classifyFailure`; ai-platform/src/invocation/index.ts:L755-L761 — all-providers-exhausted return |
...
| Side effects | `ai_attempt` 3 rows in order: `(1, "bogus-primary", "retryable_failure", "provider_unavailable")`, `(2, "bogus-primary", "retryable_failure", "provider_unavailable")` (per-attempt adapter construction, not a shared empty queue), `(3, "fake", "success")`. `routing_decision.chain` lists bogus then fake. `ai_request` → `Completed`; credit once tokens 30. |
| Code reference | ai-platform/src/invocation/index.ts:L525-L568 — chain walk and `selection_reason` derivation; ai-platform/src/worker.ts:L358-L366 — unknown-id FakeAdapter fallback, per-attempt construction |
...
| Expected outcome | `accepted` → `text_delta("Partial output…", sequence 0, provisional: true)` → `failed` `validation_failed` (`retry_safe: true`). Mechanism: attempt outcome `truncation`; `validation_failed` is retryable but `max_attempts: 1` leaves no retry; chain end with `exhaustedViaTruncation` returns `createValidationFailedError()` (never `provider_unavailable`, never authoritative `completed`). After `pushable.end()`, the worker **awaits the broker drain** so the buffered truncation chunk is relayed as provisional `text_delta` **before** `pushFailedTerminal`. It arms `ignoreBrokerSettlement = true` **before** `pushable.end()`, so when the broker afterwards sees `wasTruncated()` and would emit its own `failed` + credit, both are suppressed; the worker's `pushFailedTerminal` is the single terminal and `settleTerminal` the single credit. Assert `creditUsage` called exactly once. |
