...
1. **Seeded bootstrap admin** (created by migration `20260516100400_auth_rbac_seed.sql`, not by us): `auth.users.id = a0000000-0000-4000-8000-000000000001` (login `admin@admin`), `staff_members.id = b0000000-0000-4000-8000-000000000001`, `role = 'administrator'`, `is_bootstrap_admin = true`. Referred to below as **OWNER** (the doc's name for the first installer).
2. **Clinic bootstrap** (pre-AI clinic onboarding journey, real RPC): as OWNER, `SELECT public.bootstrap_finish_setup('Sunrise Dental Clinic', 'Main Branch', '[{"username":"nadia_karim","password":"Nadia#Karim2026!","full_name":"Nadia Karim","role":"administrator"},{"username":"omar_haddad","password":"Omar#Haddad2026!","full_name":"Omar Haddad","role":"doctor"}]'::jsonb, '{}'::jsonb, NULL, 'EGP', 'Africa/Cairo', 'MAIN', '12 Nile St, Cairo', '+201000000010', NULL);` → org **ORG** "Sunrise Dental Clinic", branch **B-MAIN** "Main Branch", staff **ADMIN** (Nadia Karim, administrator) and **DOCTOR** (Dr. Omar Haddad, doctor). Look up their `auth_user_id` from `public.staff_members`.
...
| Action | As OWNER: `SELECT public.rotate_installation_key();` |
...
|-------|---------|
| ID | S02-007 |
| Journey setup | Common journey setup; then as OWNER: `SELECT public.set_staff_active('<ADMIN staff_members.id>', false);` (real RPC) so ADMIN's row has `is_active = false`. |
...
| Action | As OWNER: `SELECT public.enroll_installation_keypair();` |
...
| Side effects | Exactly ONE new row in `ai_internal.installation_keys`: `kid = K0`, `installation_id = I0` (fresh — no prior row existed), `algorithm = 'EdDSA'`, `octet_length(public_key) = 32`, `octet_length(secret_key) = 64`, `revoked_at IS NULL`, `is_deleted = false`, `created_by = updated_by = a0000000-0000-4000-8000-000000000001` (OWNER's auth user), `valid_from`/`created_at` ≈ now. `auth_internal.base64url_encode(public_key)` of the stored row equals the returned `public_jwk.x`. NO write to `ai_internal.app_settings`, `ai_internal.ai_token_issuance`, or `public.audit_log`. |
...
| Action | As OWNER (role `authenticated`): `SELECT * FROM ai_internal.installation_keys;` |
...
| Action | As DOCTOR: `SELECT public.get_ai_availability();` |
| Expected outcome | Still exactly `{"enrolled": false, "platform_base_url": null}`. Enrolling a keypair is not platform enrollment: the flag flips only after Stage 3 platform enrollment of installation I0 plus the manual/vendor flag write (S02-025). |
| Side effects | None. |
...
| Action | As OWNER: `SELECT public.revoke_installation_key('<K0>');` |
...
|-------|---------|
| ID | S02-016 |
| Journey setup | S02-013 completed (keys exist). Caller is OWNER (authorized) so the input validation branch is what fires. |
| Action | As OWNER, two calls: `SELECT public.revoke_installation_key('');` and `SELECT public.revoke_installation_key('   ');` (whitespace-only — the `trim()` boundary). |
...
| Action | As OWNER: `SELECT public.revoke_installation_key('00000000-0000-0000-0000-000000000000');` |
...
| Action | As OWNER: `SELECT public.revoke_installation_key('<K1>');` |
...
| Action | As ADMIN: `SELECT public.rotate_installation_key();` → save new kid as K2. Then as OWNER: `SELECT public.revoke_installation_key('<K1>');` |
...
| Side effects | New row K2 (`revoked_at IS NULL`, `created_by` = ADMIN's auth user). K1 row UPDATEd (`revoked_at`, `updated_at`, `updated_by` = OWNER's auth user). End state: K0 revoked, K1 revoked, K2 active — exactly one active key, three non-deleted rows, all sharing I0. |
...
| Action | As OWNER: `SELECT public.enroll_installation_keypair();` → save new kid as KX. |
...
| Action | As ADMIN: `SELECT public.rotate_installation_key();` |
| Expected outcome | `success = true` with a new kid and `installation_id = I0`. CODE BEHAVIOR: rotate's empty-keystore guard looks up ANY non-deleted row (no `revoked_at IS NULL` filter), so `v_installation_id` is found and rotation proceeds. This CONTRADICTS orientation doc §3.1 ("Requires at least one active key row — otherwise returns INSTALLATION_NOT_ENROLLED") — see Doc-drift observations. `INSTALLATION_NOT_ENROLLED` fires only when the table has zero non-deleted rows (S02-003). |
...
|-------|---------|
| ID | S02-024 |
| Journey setup | S02-009 completed (I0/K-material exist); Stage 3 platform enrollment of installation I0 has completed on the AI platform side (the Worker returned `platform_base_url`, e.g. `http://127.0.0.1:8787`). Then [SEED] as `postgres`: `UPDATE ai_internal.app_settings SET value_json = jsonb_build_object('enrolled', true, 'platform_base_url', 'http://127.0.0.1:8787') WHERE key = 'ai.availability';` — justified because NO write RPC exists (`set_ai_availability` does not exist in any migration; verified by search); the vendor runbook performs exactly this UPDATE in production today (orientation doc §4). |
| Action | As DOCTOR (any authenticated role): `SELECT public.get_ai_availability();` |
| Expected outcome | `{"enrolled": true, "platform_base_url": "http://127.0.0.1:8787"}` — plain jsonb, readable by every authenticated role (doctor, receptionist, administrator alike). |
| Side effects | None from the read. The flag does NOT grant quotas or capabilities — platform-side entitlement remains a Stage 4 concern; the flag only unhides the Flutter AI surface. |
| Code reference | backend/supabase/migrations/20260802140000_ai_availability_flag.sql:L10-L26 — read path; backend/supabase/migrations/20260821120000_fix_get_ai_availability_security_definer.sql:L4-L11 — definer wrapper |
...
|-------|---------|
| ID | S02-027 |
| Journey setup | S02-009 completed; I0, K0, and the returned `public_jwk` were saved. (If rotation has since occurred, the handoff uses the NEWEST active key — K2 from S02-020 — with the same checks.) |
| Action | As `postgres`, reconstruct the Stage 3 enroll payload from clinic state: `SELECT jsonb_build_object('installation_id', ik.installation_id, 'kid', ik.kid, 'public_key', auth_internal.base64url_encode(ik.public_key), 'algorithm', 'EdDSA') FROM ai_internal.installation_keys ik WHERE ik.kid = '<K0>' AND ik.is_deleted = false;` and compare against the values the RPC returned in S02-009. |
| Expected outcome | The reconstructed object matches the RPC output field-for-field: `installation_id` = I0 (becomes the Stage 3 path parameter `POST /control/installations/{installation_id}/enroll` and later every AAT's `iss`), `kid` = K0 (becomes `installation_key.key_id` on the platform and the AAT header `kid`), `public_key` = the `public_jwk.x` string from S02-009 (base64url of the raw 32-byte Ed25519 public key), `algorithm` = 'EdDSA'. The handoff carries NO secret material: `secret_key` never appears in any RPC response — Stage 3 platform enrollment of installation I0 receives only the public half. |
...
1. **`rotate_installation_key` guard scope (doc wrong, code authoritative).** Orientation doc §3.1 and §5 say rotate "Requires at least one active key row — otherwise returns `INSTALLATION_NOT_ENROLLED`." The code (`20260801120100...sql:L103-L115`) looks up ANY non-deleted row with no `revoked_at` filter, so rotate SUCCEEDS when all keys are revoked but rows remain (scenario S02-023). `INSTALLATION_NOT_ENROLLED` is reachable only with a completely empty (or fully soft-deleted) keystore (S02-003).
2. **`issue_ai_token` error shape misdescribed.** Orientation doc §8.3.2 expects `success = false, error_code = 'INSTALLATION_NOT_ENROLLED'` (an `rpc_result` envelope) when minting before enrollment. `auth_internal.issue_ai_token` returns `text` and RAISEs `EXCEPTION 'INSTALLATION_NOT_ENROLLED'` (P0001) — PostgREST surfaces an HTTP error, not an envelope. (Stage 6 territory, but the drift lives in the Stage 2 doc.)
