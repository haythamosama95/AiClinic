...
|-------|---------|
| ID | S02-024 |
| Journey setup | S02-009 completed (I0/K-material exist); Stage 3 platform enrollment of installation I0 has completed on the AI platform side (the Worker returned `platform_base_url`, e.g. `http://127.0.0.1:8787`). Common journey setup has ADMIN and DOCTOR only, so as ADMIN (JWT overlay includes `organization_id` / `staff_member_id` as in S02-007): `SELECT public.create_staff_account('s02_reception', …, 'receptionist', …);` and look up that staff row's `auth_user_id` for the receptionist read. |
| Action | As BOOT: `SELECT public.set_ai_availability(true, 'http://127.0.0.1:8787');` then as DOCTOR, as ADMIN, and as the receptionist just provisioned: `SELECT public.get_ai_availability();` |
| Expected outcome | `set_ai_availability` returns `success = true`, `data = {"enrolled": true, "platform_base_url": "http://127.0.0.1:8787"}`. Each `get_ai_availability` returns the same plain `jsonb` — readable by every authenticated role (doctor, receptionist, administrator alike). |
