...
| Action | Evaluate the worker module (boot attempts `setCapabilityRegistry` without `{ replace: true }`, which throws `CapabilityRegistry is already installed; pass { replace: true } to replace`; the catch swallows it). Then `GET /v1/capabilities` with a valid AAT whose entitlement allows `test.echo` **and** `clinic.visit_summary` (Stage 3/4/6 happy paths with both ids granted so stage 3 passes). Then `POST /v1/requests` naming `clinic.visit_summary`. |
| Expected outcome | Boot completes; the harness registry survives (the "already installed" throw is caught and swallowed — this is the **only** boot catch that does not rethrow). Discovery HTTP 200 lists `test.echo@9.9.9` and does **not** list `clinic.visit_summary@1.0.0`. The invoke reaches stage 5 and returns HTTP 404 `capability_unknown` because entitlement passed and the registry misses. A **non-granted** unknown capability is 403 `forbidden_capability` (stage 3 before resolve); 404 `capability_unknown` is reachable only when entitlement passes and the registry misses. |
...
| Action | `GET /v1/capabilities` with the AAT (warm pass, HTTP 200 lists the capability). Re-entitle of an **active** entitlement is 409 `not_pending` (S04-049/051) — no control route updates `allowed_capabilities` after activate. Instead cohort-activate `clinic.visit_summary@2.0.0` onto the installation (`POST /control/capabilities/clinic.visit_summary/versions/2.0.0/activate` with `installation_ids: [I0]`; the v2 manifest's `minimumPlanTier` is `enterprise`, so discovery filters it out), then immediately re-issue `GET /v1/capabilities` with the same AAT (cache cleared / TTL 0 so the next GET re-reads D1). |
...
| Side effects | The cohort-activate writes its own grant row (owned by Stage 4); this scenario adds no writes beyond that. |
...
| Expected outcome | `load` throws before any hash comparison: respectively `Missing manifest group: Governance`, `Malformed manifest Governance.retentionClass: diagnostic horizon must be 1–90 days`, or `Malformed manifest group: Routing` (exact-key validation fires **before** the provider/model naming ban when `Routing.provider` is added). This answers the reachability question for malformed manifests: at runtime the boot `try/catch` (S00-007) swallows a load failure and degrades to an empty registry; the **enforcing** surface is this build gate, which aborts the pipeline. |
| Side effects | None. |
...
| Action | (1) `GET /v1/capabilities` with the AAT → warms the isolate cache with the entitlement/grant rows. (2) Re-entitle of an **active** entitlement is 409 `not_pending` (S04-049/051). Instead cohort-activate `clinic.visit_summary@2.0.0` via `POST /control/capabilities/clinic.visit_summary/versions/2.0.0/activate` **without** clearing the isolate cache (the activate UPDATEs the installation grant 1.0.0 → 2.0.0 in D1; v2 is filtered out of discovery by `enterprise` `minimumPlanTier`). (3) Immediately (within 100 ms) re-issue `GET /v1/capabilities`. (4) After the TTL has elapsed, issue it a third time. |
| Expected outcome | (1) HTTP 200 listing `clinic.visit_summary@1.0.0`. (3) HTTP 200 **still listing** the capability — the cached grant row is served inside its TTL (stale read is the designed behavior, not a bug). (4) HTTP 200 with `{"manifests":[]}` — the TTL expired, `consult` evicted the entry, D1 was re-read, and the 2.0.0 grant is not advertised. The ETag at (4) differs from (1)/(3). |
| Side effects | Cohort-activate writes are owned by Stage 4. This scenario adds only cache entries (isolate-local memory, no D1/DO/R2 writes). |
