...
12. **Rejection tally coverage is uneven by stage**: stages 2–4 and admission failures call `recordGuardRejection`; capability (5) now tallies rejections (**C-21**, partial — context/preflight stages 6–7 still pending). `platform_counter` therefore still undercounts guard rejections from stages 6–7 beyond the documented isolate-eviction lower bound.
