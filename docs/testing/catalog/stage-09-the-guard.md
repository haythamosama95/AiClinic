...
## Scenario S09-027 — Stage 3 entitlement: missing entitlement row returns stage-3 internal_error GuardFailure
...
| Code reference | `ai-platform/src/entitlement/index.ts:L154-L175` — entitlement miss → `internal_error`; `ai-platform/src/pipeline/index.ts:L349-L359` — stage-3 `fail()` wiring |
...
|-------|---------|
| ID | S09-078 |
| Journey setup | Baseline B0 (default `request_quota: 1000`) plus a second installation enrolled and entitled with `request_quota: 0` (real Stage 4 entitle, same period). ConfigCache cleared. Register 5 #28: in-pool `env.DO` is frozen, so `wrapDurableObjectNamespace` / `installEnvOverrides` cannot take the Quota DO down on `SELF.fetch` (siblings S09-075/076/077/079 stay skipped). Drive `runAdmission` with a throwing DO namespace (`wrapDurableObjectNamespace` `fetchThrow` — the documented #28 seam SX-013/017/018 already use). The default-quota installation is `grace_admitted` first, proving the seam entered `admitUnderGrace` (a second `provisionHappyPath` cannot republish; live entitle cannot restore `request_quota` on the exhausted installation — 409 `not_pending`). The ledger check counts `ai_request` rows in the period: `COUNT(*) >= 0` is immediately true — no request burning required. |
| Action | `runAdmission` on the default-quota installation (idempotency key `grace-control`), then `runAdmission` on the `request_quota: 0` installation with idempotency key `grace-exhausted`, both with the throwing DO binding. |
| Expected outcome | Control: `grace_admitted` (one pending grace row). Exhausted: `{ ok: false, code: "quota_exhausted", periodReset: <entitle period_end> }` — the same taxonomy code and `period_reset` the wire maps to HTTP 429 (S09-070). **No grace row is queued for the exhausted installation** — `isLedgerQuotaExhausted` runs inside `admitUnderGrace` before the capped insert. |
| Side effects | Exhausted installation: no `grace_admission_queue` row; no `ai_request` row. Tally +1 `quota_exhausted`. Control installation: one pending grace row. |
| Code reference | `ai-platform/src/admission/index.ts:L506-L517` — ledger exhaustion check in `admitUnderGrace`; `ai-platform/src/admission/index.ts:L264-L301` — `isLedgerQuotaExhausted`; Register 5 #28 |
...
| Side effects | The stage-9 `ai_request` row transitions `Accepted → Failed` (`completed_at` set, `terminal_error_code = "internal_error"`). The DO admission reservation is released (mirrors stage-9 failure): `inFlight` decremented, idempotency and jti entries cleared. No `usage_event`, no R2 envelope. Reset the artifact overlay afterwards. |
| Code reference | `ai-platform/src/pipeline/index.ts:L508-L533` — stage-10 wiring, terminal-state record, and release on failure; `ai-platform/src/prompt/composer.ts:L336-L340` — missing-fragment failure; `ai-platform/src/prompt/registry.ts:L69-L78` — test overlay seam |
...
3. **DO wrong-kind success body (`kind !== "admission"` → `internal_error`, folded into S09-079)** — the real `GatewayObject` never emits a well-formed 200 with a foreign `kind` on the admission path; producing one needs a DO namespace double scripted to return `{kind:"credit", …}` on the admission fetch. Full-path `POST /v1/requests` DO-outage journeys (S09-075/076/077/079) remain Register 5 #28 skips. S09-078 is the ledger-exhaustion sibling: it injects `runAdmission` + throwing DO (the documented #28 seam) rather than skipping.
