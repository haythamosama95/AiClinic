...
| Expected outcome | SSE: `accepted` → `text_delta("Partial output…", sequence 0, provisional: true)` → `failed` `validation_failed` (`retry_safe: true`). The worker drains buffered provisional chunks before `pushFailedTerminal` (same relay as S10-028 / architecture provisional tokens). The broker's own `handleFailed("validation_failed")` (wasTruncated) still fires after `ignoreBrokerSettlement = true`, so **its** event/credit/journal are dropped — not the `text_delta`. `ai_request` `state 'Failed'`, `terminal_error_code 'validation_failed'` (written by the worker's `recordTerminalState`, since the broker journal sink was suppressed). One `ai_attempt`: `outcome 'truncation'`, `tokens_in 10`, `tokens_out 20`, `cost 0.005`, `error_code` NULL (truncation records usage but no error code). One `usage_event` `{ tokens: 30, cost: 0.005 }` — accrued partial usage from the truncated result. DO: exactly one credit, from the worker, `partial: false` (`validation_failed` consumesQuota `"Yes"`), `idempotencyState "failed"`, usage `{ tokens: 30, cost: 0.005 }`. Envelope `result.finishReason = "validation_failed"`. |
...
## Scenario S11-014 — Idempotent replay after Failed replays the original taxonomy code with no new settlement
...
| Expected outcome | SSE terminal `failed` whose `data.code` is `provider_unavailable` — replay echoes the `terminalErrorCode` persisted in the DO idempotency entry at credit time. `retry_safe: true` (`provider_unavailable` retryable "Yes"). Zero new settlement: counts, envelope, and `ai_request` row unchanged; `terminal_error_code` still `provider_unavailable` on the original row. DO counters unchanged. |
| Side effects | MUST: no credit, no journal, no new rows. MUST NOT: the replay must not emit `"Prior request completed."` (that placeholder is only for `completed` prior states); no second `usage_event` for RIDF. |
| Code reference | ai-platform/src/worker.ts — `replayIdempotentTerminal` failed branch; ai-platform/src/quota-do/index.ts — `terminalErrorCode` on idempotency entry; ai-platform/src/worker.ts — `settleTerminal` forwards code on `idempotencyState: "failed"` |
...
|-------|---------|
| ID | S11-027 |
| Journey setup | Seeded clinic as S11-022. Optimistic concurrency compares `visit_clinical_notes.updated_at`, not `visits.updated_at`. In the single-transaction SQL harness `now()` is stable, so the S11-022 note timestamp equals setup T0; send `p_expected_updated_at` = T0 − 1 s so `IS DISTINCT FROM` fires. Fresh reference `'9P4R-SXTC'`. |
...
|-------|---------|
| ID | S11-028 |
| Journey setup | [SEED] Register a test target: insert `('test_no_record_id', 'test_domain_no_id', 'visit_clinical_notes')` into `ai_internal.acceptance_targets` and create `public.test_domain_no_id(p_unused text DEFAULT NULL) returns rpc_result` as a function that performs a real write (e.g. inserts an audit_log row) then returns `rpc_success('{}'::jsonb)` — no `record_id`/`visit_id`/`id` key. A **zero-arg** stub is rejected earlier as `rpc_error('INTERNAL_ERROR', 'Domain function is not configured.')` (`v_nargs = 0`) and never reaches the RAISE. Justification: every shipped domain function returns an id, so the no-id branch is unreachable with the production registry alone. |
...
| Side effects | MUST: authenticated SELECT on `ai_internal.acceptance_targets` is denied at **schema** level — 42501 `permission denied for schema ai_internal` (`REVOKE ALL ON SCHEMA ai_internal FROM authenticated`), not a table-level deny. MUST NOT: no authenticated path to the dynamic dispatcher; no cross-org visibility of acceptance provenance. |
