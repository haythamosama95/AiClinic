...
**Frontend automation:** 170 tests in `test/unit/setup/`, `test/widget/setup/`, plus auth guard & permission matrix suites (see run command above).

**Not automated (by design):** release-mode dev panel hiding, step-transition animation timing, router redirect refresh integration, debug hot-reload persistence, 320px viewport (known step-indicator overflow — scroll view present but layout overflows).
...
| `step indicator shows four stages`        | Modal renders            | Organization, Branch, Staff, Complete labels/steps visible | Missing step       | 📋      |
...
| `setup modal has floating card styling`        | Modal visible            | White card, 24px radius, `ShadowTokens.shadowLg`                 | Flat unstyled panel                 | 📋        |
