...
## 3. S06-019 side-effect fingerprints (xmax / public max(updated_at))
...
- CODE issuer SELECTs keystore / staff / branch / RBAC rows and INSERTs the
  ...
  tables.
- A hash over `installation_keys.xmax` is not a business-write observation:
  row xmax can move from snapshot / lock visibility without an UPDATE.
  Dropped from the keys fingerprint; kid + `revoked_at` + `updated_at` are
  kept.
- `max(updated_at)` on `public.staff_members` / `roles_permissions` /
  `branches` is not a CODE-guaranteed observable for “issuer does not write
  public.*”. Replaced with row counts only.
