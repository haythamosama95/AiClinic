-- Extend search_patients list items with last completed visit and next upcoming appointment.
...
BEGIN
  ...
  SELECT COALESCE(
    jsonb_agg(
      ...
      ORDER BY sub.name_match_rank, sub.full_name
    ),
    '[]'::jsonb
  )
  INTO v_items
  FROM (
    ...
    ORDER BY name_match_rank ASC, p.full_name ASC
