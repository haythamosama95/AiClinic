-- Dev reset must remove patient health tracking rows before patients (014 encounter workspace FKs).
...
BEGIN
  ...
  END IF;
  -- Visit documentation (013): child rows reference visits without ON DELETE CASCADE.
  ...
  -- Patient health tracking (014): child rows reference patients without ON DELETE CASCADE.
