-- Allow scheduled appointments to move to another branch during update (V1-4 edit flow).

DROP FUNCTION IF EXISTS public.update_appointment(uuid, uuid, uuid, timestamptz, int, timestamptz, text);
DROP FUNCTION IF EXISTS auth_internal.update_appointment(uuid, uuid, uuid, timestamptz, int, timestamptz, text);
...
BEGIN
  ...
  IF p_branch_id IS NOT NULL AND p_branch_id <> v_appt.branch_id THEN
    IF v_appt.status <> 'scheduled' THEN
      RETURN public.rpc_error(
        'INVALID_INPUT',
        'Branch cannot be changed after the appointment is confirmed.'
      );
    END IF;

    IF NOT (v_branch_id = ANY (public.jwt_branch_ids())) THEN
      RETURN public.rpc_error('INVALID_BRANCH', 'The selected branch is not valid for this session.');
    END IF;
  END IF;

  IF p_doctor_id IS NOT NULL THEN
    PERFORM auth_internal.assert_appointment_doctor(p_doctor_id, v_branch_id);
  END IF;
  ...
  v_duration := COALESCE(p_duration_minutes, auth_internal.resolve_appointment_default_duration(v_branch_id));
  ...
  IF v_appt.status <> 'scheduled' THEN
    ...
    IF v_branch_id <> v_appt.branch_id THEN
      RETURN public.rpc_error(
        'INVALID_INPUT',
        'Branch cannot be changed after the appointment is confirmed.'
      );
    END IF;
  ELSE
    IF NOT auth_internal.appointment_within_branch_working_hours(v_branch_id, v_start, v_end) THEN
      RETURN public.rpc_error('INVALID_INPUT', 'Appointment must be within branch working hours.');
    END IF;

    IF auth_internal.patient_has_same_day_appointment(v_branch_id, p_patient_id, v_start, v_appt.id) THEN
      RETURN public.rpc_error(
  ...
  IF auth_internal.appointment_has_overlap(v_branch_id, v_effective_doctor_id, v_start, v_end, v_appt.id) THEN
...
GRANT EXECUTE ON FUNCTION auth_internal.update_appointment(
  uuid, uuid, uuid, timestamptz, int, timestamptz, text, uuid
) TO authenticated;
