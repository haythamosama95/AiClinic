-- Dev-only bulk import for organization investigations catalog (used by Flutter Fill Dummy Clinic).
...
CREATE OR REPLACE FUNCTION public.dev_seed_investigations_catalog(p_names text[])
RETURNS public.rpc_result
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
AS $$ SELECT auth_internal.dev_seed_investigations_catalog(p_names); $$;

GRANT EXECUTE ON FUNCTION auth_internal.dev_seed_investigations_catalog(text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.dev_seed_investigations_catalog(text[]) TO authenticated;
