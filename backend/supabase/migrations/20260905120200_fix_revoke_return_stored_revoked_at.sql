-- Fresh-revoke success payload must return the stored revoked_at, not a new clock_timestamp().
...
BEGIN
  ...
  WHERE ik.kid = p_kid
  RETURNING ik.revoked_at INTO v_revoked_at;

  RETURN public.rpc_success(
    jsonb_build_object('kid', p_kid, 'revoked_at', v_revoked_at)
  );
EXCEPTION
