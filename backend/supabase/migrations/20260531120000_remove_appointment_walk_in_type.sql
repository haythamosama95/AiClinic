-- V1-4: Remove walk_in from appointment_type; planned-only appointments.

UPDATE public.appointments
SET type = 'planned'
WHERE type::text = 'walk_in';

CREATE TYPE public.appointment_type_planned_only AS ENUM ('planned');

ALTER TABLE public.appointments
  ALTER COLUMN type DROP DEFAULT;

ALTER TABLE public.appointments
  ALTER COLUMN type TYPE public.appointment_type_planned_only
  USING type::text::public.appointment_type_planned_only;

ALTER TABLE public.appointments
  ALTER COLUMN type SET DEFAULT 'planned'::public.appointment_type_planned_only;

DROP TYPE public.appointment_type;

ALTER TYPE public.appointment_type_planned_only RENAME TO appointment_type;

-- Simplify create_appointment type validation (planned-only enum).
