-- Allow clinic administrators to create/assign owner accounts (not only existing owners).
-- Add atomic bootstrap_finish_setup so org/branch/staff creation rolls back together on failure.
...
BEGIN
  ...
  IF p_role = 'owner' THEN
    IF auth_internal.owner_exists() THEN
      IF v_caller.role NOT IN ('owner', 'administrator') THEN
        RETURN public.rpc_error(
          'FORBIDDEN_OWNER_CREATE',
          'Only clinic owners and administrators may create additional owner accounts.'
        );
      END IF;
...
BEGIN
  ...
  IF p_role = 'owner' AND v_old_role <> 'owner' THEN
    IF auth_internal.owner_exists() AND v_caller.role NOT IN ('owner', 'administrator') THEN
      RETURN public.rpc_error(
        'FORBIDDEN_OWNER_CREATE',
        'Only clinic owners and administrators may assign the owner role.'
      );
    END IF;
  END IF;
...
BEGIN
  ...
  IF v_owner_count = 0 THEN
    RETURN public.rpc_error('INVALID_INPUT', 'At least one owner account is required to finish setup.');
  END IF;
...
REVOKE EXECUTE ON FUNCTION public.bootstrap_finish_setup(
  text, text, jsonb, jsonb, text, text, text, text, text, text, text
) FROM anon, PUBLIC;
GRANT EXECUTE ON FUNCTION public.bootstrap_finish_setup(
  text, text, jsonb, jsonb, text, text, text, text, text, text, text
) TO authenticated;
GRANT EXECUTE ON FUNCTION auth_internal.bootstrap_finish_setup(
  text, text, jsonb, jsonb, text, text, text, text, text, text, text
) TO authenticated;
